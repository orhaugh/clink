...
// Standard RFC 4648 base64 (alphabet A-Za-z0-9+/ with '=' padding). The Pub/Sub
// REST API carries message payloads as base64 in the JSON "data" field, so the
// publish sink encodes and the pull source decodes. Kept generic + header-only
// so both halves share one implementation.
