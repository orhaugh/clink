...
// over forst:// restores, not only in-memory ones.
TEST(ForStStateBackend, StateFingerprintsSurviveSnapshotAndRestore) {
    auto dir = std::filesystem::temp_directory_path() / "clink_forst_fingerprints";
    std::filesystem::remove_all(dir);
    std::filesystem::create_directories(dir);
    ForStStateBackend backend(
        ForStStateBackend::Options{.path = (dir / "db").string(), .create_if_missing = true});
...
// exported forst checkpoint restored into the in-memory reference reader
// carries them - the pre-deploy savepoint reader sees real stamps.
TEST(ForStStateBackend, ArrowExportsCarryStateFingerprints) {
    auto dir = std::filesystem::temp_directory_path() / "clink_forst_fingerprints_export";
    std::filesystem::remove_all(dir);
    std::filesystem::create_directories(dir);
    ForStStateBackend backend(
        ForStStateBackend::Options{.path = (dir / "db").string(), .create_if_missing = true});
...
TEST(ForStStateBackend, ClearFingerprintWholeOpPersistsAcrossReopen) {
    auto dir = std::filesystem::temp_directory_path() / "clink_forst_fp_clear";
    std::filesystem::remove_all(dir);
    std::filesystem::create_directories(dir);
    const auto db_path = (dir / "db").string();
    {
        ForStStateBackend backend(
            ForStStateBackend::Options{.path = db_path, .create_if_missing = true});
    ...
    ForStStateBackend reopened(
        ForStStateBackend::Options{.path = db_path, .create_if_missing = false});
