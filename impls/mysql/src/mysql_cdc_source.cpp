...
public:
    ...
    void open() override {
        ...
        // strips the trailing CRC correctly, then ask for periodic heartbeats so
        // an idle fetch returns within heartbeat_ms (bounds cancel + checkpoint).
        ...
        start_position_ = cur_file_ + ":" + std::to_string(cur_pos_);
    ...
    bool produce(Emitter<std::string>& out) override {
        ...
        if (this->cancelled() || rpl_ == nullptr) {
            return false;
        }
