...
// Builds a RocksDBStateBackend rooted at <base>/<subtask_idx>. When the
// spec carries a restore location, wire out.restore_from to this subtask's
// checkpoint dir so LocalExecutor calls backend->restore() before any
// operator runs. snapshot() writes each checkpoint to "<db_path>.cp-<id>"
// (a sibling of the db dir); restore() re-homes the DB onto it. This
// recovers a same-parallelism restart (the common failover case) fully,
// including operator state.
//
// Restore covers same-parallelism restart and full rescale (scale-up AND
// scale-down): restore() re-homes onto the first assigned parent's checkpoint,
// iterate-merges any further parents, and narrows keyed rows by the key-group
// filter while preserving operator-state rows (source offsets).
BuiltStateBackend build_rocksdb(const StateBackendSpec& spec) {
    auto [_, base_path] = split_uri(spec.uri);
    if (base_path.empty()) {
        throw std::runtime_error("rocksdb scheme requires a path");
    }
    ...
    // RocksDB::Open only mkdirs the leaf dir, so the parents must exist
    // first (mirrors build_changelog_rocksdb). Without this, opening
    // <base>/<subtask_idx> fails whenever <base> isn't already present.
    std::error_code mkdir_ec;
    std::filesystem::create_directories(subtask_dir, mkdir_ec);

    clink::RocksDBStateBackend::Options opts;
    opts.path = subtask_dir.string();
    opts.create_if_missing = true;

    BuiltStateBackend out;
    out.backend = std::make_shared<clink::RocksDBStateBackend>(std::move(opts));
    ...
    if (want_restore) {
        ...
        // parent_count names the contiguous parents this new subtask inherits
        // (scale-up = 1 parent, scale-down = several). snapshot() wrote each
        // checkpoint to "<db_path>.cp-<id>". restore() re-homes onto the first
        // and iterate-merges the rest; the key-group filter (from
        // LocalExecutor) narrows the keyed rows to this subtask's slice. The
        // paths ride snap.bytes newline-separated.
        ...
        for (std::uint32_t i = 0; i < parent_count; ++i) {
            ...
            if (!std::filesystem::exists(dir)) {
                throw std::runtime_error(
                    "rocksdb scheme: restore requested but checkpoint dir not found: " + dir);
            }
            if (!joined.empty()) {
                joined.push_back('\n');
            }
...
// changelog+rocksdb://<dir>: the production-grade large-state config.
//   * Inner backend: RocksDBStateBackend at <dir>/<subtask_idx>/inner.
//     Reads/writes/scans go to RocksDB so state can exceed RAM.
//   * Materialization store: FileMaterializationStore at
//     <dir>/<subtask_idx>/mat. Each materialization writes a single
//     <dir>/<subtask_idx>/mat/mat-<ckpt_id>.bin file; the Snapshot
//     blob carries only the path handle.
//
// The previous meaning (InMemory inner + RocksDB-store materialization)
// was a corner case - if RocksDB is available, you'd rather use it
// as the inner. The RocksDbMaterializationStore class remains exposed
// via its public header for code that explicitly wants the
// InMemory+RocksDB combo by hand.
BuiltStateBackend build_changelog_rocksdb(const StateBackendSpec& spec) {
    auto [_, base_path] = split_uri(spec.uri);
    if (base_path.empty()) {
        throw std::runtime_error("changelog+rocksdb scheme requires a path");
    }
    const bool want_restore = !spec.restore_uri.empty() && spec.restore_checkpoint_id != 0;
    const std::uint32_t parent_count =
        spec.restore_from_parent_count == 0 ? 1 : spec.restore_from_parent_count;
    ...
    // RocksDB::Open only mkdirs the leaf - the subtask dir must exist
    ...
    opts.create_if_missing = true;
    auto inner = std::make_shared<clink::RocksDBStateBackend>(std::move(opts));
    ...
    if (want_restore) {
        ...
        // packs them; restore() splits + merges (the inner RocksDB folds the
        // parents' checkpoint dirs via combine_snapshots) and narrows by the
        // key-group filter. Each parent's blob, mat file, and inner .cp-<id>
        // dir live under that parent's source subtask dir.
        const bool is_rescale =
            spec.restore_from_subtask_idx != std::numeric_limits<std::uint32_t>::max();
        const std::uint32_t src_first =
            is_rescale ? spec.restore_from_subtask_idx : spec.subtask_idx;
        ...
        for (std::uint32_t i = 0; i < parent_count; ++i) {
            ...
            if (!in) {
                throw std::runtime_error(
                    "changelog+rocksdb scheme: restore requested but snapshot not found: " +
                    blob.string());
            }
...
void install() {
    clink::StateBackendFactory::default_instance().register_scheme("rocksdb", &build_rocksdb);
    clink::StateBackendFactory::default_instance().register_scheme("changelog+rocksdb",
                                                                   &build_changelog_rocksdb);
}
