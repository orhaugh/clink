...
/**
 * In-process clink-vs-Flink bench (Flink side).
 *
 * Pipeline mirrors the clink side in
 * ../../clink-job/inproc_pipeline.cpp: a bounded synthetic source
 * emits rich Event POJOs, the pipeline keyBy-windows-aggregates them
 * into EventStats, and a counting sink prints the final count. The
 * source closes after N events so the job completes naturally; this
 * lets us measure wall-clock from job submission to job completion.
 *
 ...
 *   BENCH_CHECKPOINT_DIR (default /tmp/flink-parallel-ckpts)
 */
public final class ParallelPipeline {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final int parallelism =
                Integer.parseInt(System.getenv().getOrDefault("BENCH_PARALLELISM", "4"));
        final String ckptDir =
                System.getenv().getOrDefault("BENCH_CHECKPOINT_DIR", "/tmp/flink-parallel-ckpts");
        ...
        env.execute("flink-parallel-bench");
    }
}
