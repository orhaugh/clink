...
// crash + restore; v2 will kill the worker mid-run and verify
...
inline void define_job(clink::api::Pipeline& env) {
    ...
    const auto parallelism = static_cast<std::uint32_t>(env_i64("BENCH_PARALLELISM", 4));
    ...
    env.registry().register_key_extractor<Event>("by_key", [](const Event& e) { return e.key; });
