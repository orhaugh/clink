// Sliding-window clink-vs-Flink bench: the clink side.
//
// Pipeline:
//   in-proc source -> assign_timestamps_monotonic -> key_by ->
//   sliding_window(size=1s, slide=250ms) -> aggregate(EventStats)
//   -> counting sink.
//
// The source generates rich Event records (~2 KB each) at full speed
// in process. Source closes after BENCH_RECORDS events. Each record
// fans out into size/slide = 4 overlapping windows, so the per-record
// state-update cost is multiplied by 4. Expected sink count:
//   keys * ((windows * 1000ms) / slide) panes.
// At default BENCH_KEYS=1000, BENCH_WINDOWS=60, slide=250: 240_000 panes.
//
...
public:
    SyntheticEventSource(std::int64_t total,
        ...
        : total_(total), keys_(keys), windows_(windows), payload_size_(payload_size) {
        auto body = std::make_shared<EventBody>();
...
inline void define_job(clink::api::StreamExecutionEnvironment& env) {
    ...
    // Register the synthetic source as a custom op_type. Built once per
    // subtask via the BuildContext-aware factory (parallelism > 1 case
    // would shard the cursor; v1 of this bench is par=1).
    const std::string source_op = "bench.synthetic_event_source";
    env.registry().register_source<Event>(
        source_op,
        [records, keys, windows, payload_size](const clink::plugin::BuildContext& /*ctx*/) {
            return std::make_shared<SyntheticEventSource>(records, keys, windows, payload_size);
        });
...
CLINK_REGISTER_JOB("sliding-window-bench-pipeline",
                   "1.0",
                   "in-proc synthetic source -> key_by -> sliding 1s/250ms -> aggregate -> counting sink",
                   bench::define_job);
