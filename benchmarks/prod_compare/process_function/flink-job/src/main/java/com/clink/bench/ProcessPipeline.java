...
/**
 * Process-function clink-vs-Flink bench (Flink side).
 *
 * Pipeline mirrors the clink side in clink-job/process_pipeline.cpp:
 * a bounded synthetic source emits rich Event POJOs, the pipeline
 * keyBy + KeyedProcessFunction (ValueState running count + ListState
 * last 8 values) emits one ProcessStats per record, counting sink
 * prints the total count. Exercises the canonical KeyedProcessFunction
 * + multi-slot state path that production business logic leans on.
 */
public final class ProcessPipeline {
    ...
    public static final class ProcessStats implements Serializable {
        private static final long serialVersionUID = 1L;
        public long key;
        public long count;
        public double last8Avg;
    ...
    private static final class StatefulFn
            extends KeyedProcessFunction<Long, Event, ProcessStats> {
        private static final long serialVersionUID = 1L;
        private transient ValueState<Long> countState;
        private transient ListState<Long> last8State;

        @Override
        public void open(org.apache.flink.api.common.functions.OpenContext oc) {
            countState = getRuntimeContext().getState(
                    new ValueStateDescriptor<>("count", Long.class));
            last8State = getRuntimeContext().getListState(
                    new ListStateDescriptor<>("last8", Long.class));
        }

        @Override
        public void processElement(Event e, Context ctx, Collector<ProcessStats> out)
                throws Exception {
            ...
            stats.key = e.key;
            stats.count = next;
            stats.last8Avg = window.isEmpty() ? 0.0 : sum / window.size();
            stats.latestPayload = e.payload;
            out.collect(stats);
        }
    }

    private static final class CountingSink implements SinkFunction<ProcessStats> {
        private static final long serialVersionUID = 1L;
        private long count = 0;
        private boolean printedFinal = false;

        @Override
        public void invoke(ProcessStats value, Context context) {
            count++;
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String ckptDir =
                System.getenv().getOrDefault("BENCH_CHECKPOINT_DIR", "/tmp/flink-process-ckpts");
        ...
        env.fromSource(
            ...
            .process(new StatefulFn())
            .returns(Types.POJO(ProcessStats.class))
            .addSink(new CountingSink());

        env.execute("flink-process-function-bench");
    }
}
