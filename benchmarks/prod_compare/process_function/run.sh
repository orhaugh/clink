#!/usr/bin/env bash
# Process-function clink-vs-Flink bench driver.
#
# Pipeline: bounded synthetic source -> keyBy -> KeyedProcessFunction
# (ValueState running count + ListState last 8 values) -> counting
# sink. One emit per input record; expected pane count = BENCH_RECORDS.
...
CLINK_CKPT_DIR="${CLINK_CKPT_DIR:-/tmp/clink-process-ckpts}"
FLINK_CKPT_DIR="${FLINK_CKPT_DIR:-/tmp/flink-process-ckpts}"
...
JAR="$ROOT/flink-job/target/flink-process-pipeline.jar"
...
cmake --build "$CLINK_ROOT/build" --parallel 10 \
    --target clink_process_bench_pipeline clink_node clink_submit_job >/dev/null
SO="$CLINK_ROOT/build/benchmarks/process_bench_pipeline.so"
...
BENCH_CHECKPOINT_DIR="$FLINK_CKPT_DIR" \
    java -cp "$JAR" com.clink.bench.ProcessPipeline >"$RESULTS/flink.log" 2>&1
FLINK_END=$(date +%s.%N)
...
CLINK_PLAN_FUSE_PAR1=1 \
    "$CLINK_ROOT/build/clink_node" --role=coordinator --port=7152 \
...
CLINK_WB_STATE_CACHE="${CLINK_WB_STATE_CACHE:-0}" \
    "$CLINK_ROOT/build/clink_node" --role=worker \
        --coordinator-host=127.0.0.1 --coordinator-port=7152 \
        --id=worker-process --slots=16 \
...
# Process-function bench: KeyedProcessFunction emits one ProcessStats
# per input record, so expected emit count == BENCH_RECORDS.
EXPECTED_PANES="$RECORDS"
