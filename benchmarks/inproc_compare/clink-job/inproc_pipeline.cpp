// In-process clink-vs-Flink bench: the clink side.
//
// Pipeline (see ../pipeline.md):
//   in-proc source -> assign_timestamps_monotonic -> key_by ->
//   tumbling_window(1s) -> aggregate(EventStats) -> counting sink.
//
// The source generates rich Event records (~2 KB each) at full speed
// in process. Source closes after BENCH_RECORDS events. Pipeline
// naturally drains, all windows fire on the final Watermark::max(),
// sink counts to N_keys * N_windows + ~stragglers, job completes.
//
