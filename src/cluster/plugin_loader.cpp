...
PluginLoadResult PluginLoader::load_into(const std::string& so_path,
    ...
    if (auto it = modules_.find(so_path); it != modules_.end()) {
        ...
        auto reject_unregistered = [&](std::string msg) {
            ::dlclose(handle);
            result.error = std::move(msg);
            return result;
        };

        // Fingerprint + version symbols are OPTIONAL: a plugin built before
        // the fingerprint gate falls back to the exact-hash comparison.
        ...
        if (auto err = check_plugin_abi(abi_in); !err.empty()) {
            return reject_unregistered(err);
        }

        const char* plugin_triple = triple_fn();
        if (plugin_triple == nullptr || std::strcmp(plugin_triple, cluster_target_triple()) != 0) {
            return reject_unregistered(
                std::string{"plugin target-triple mismatch: plugin reports '"} +
