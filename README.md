...
| Live rescale               | change one operator's parallelism on a running job (`clink rescale-op`): drain, replan, redeploy from the last completed checkpoint. Integer factors, declared `.rescalable(min, max)` bounds, and a stop rather than a seamless cutover - see [rescale](docs/internals/fault-tolerance-and-rescale.md) |
...
| Autoscaler                 | load-driven rescale trigger (`autoscaler.hpp`); requests route through per-operator rescale, so each one costs the job a restart from checkpoint |
