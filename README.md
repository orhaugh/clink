...
| Checkpoint rescale         | key-group repartitioning on deploy; scale-up and scale-down for file / RocksDB / changelog backends without full replay. Caveats: sources must store offsets as operator-state to survive rescale, and rescaling a *running* job is limited - see [rescale limits](docs/internals/fault-tolerance-and-rescale.md) |
...
| Autoscaler                 | load-driven rescale trigger (`autoscaler.hpp`); requests route through per-operator rescale, which a planned job cannot execute today |
