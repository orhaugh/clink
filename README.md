...
[![changelog](https://img.shields.io/badge/changelog-v0.7.0-lightgrey.svg)](CHANGELOG.md)
...
| Live rescale               | change one operator's parallelism on a running job (`clink rescale-op`): when every edge into and out of the operator is keyed or parallelism-mismatched, the change is a hot cutover at one checkpoint barrier - sources do not rewind, unaffected operators keep running, sink transaction cycles do not reset. Ineligible requests, and any failed cutover, fall back to drain / replan / redeploy from the last completed checkpoint. Integer factors, declared `.rescalable(min, max)` bounds - see [rescale](docs/internals/fault-tolerance-and-rescale.md) |
...
| Autoscaler                 | load-driven rescale trigger (`autoscaler.hpp`); requests route through per-operator rescale, so an eligible operator scales via the hot cutover and anything else costs a restart from checkpoint |
...
The current release is v0.7.0; changes land in the
