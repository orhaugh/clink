...
//
//   HTTP-4a/b: /metrics on JM and TM returns Prometheus text exposition,
//              with at least the process-level metric names from
//              process_metrics.hpp. After a TM registers, jm_tms_registered
//              shows up; after a subtask runs, tm_subtasks_*_total bumps.
//   HTTP-4c:   /api/v1/logs returns a JSON `{"logs": [...]}` with the
//              cluster-lifecycle events we emit (TM register on JM,
//              subtask start on TM).
//   federated: GET /api/v1/tms/:id/metrics and /api/v1/tms/:id/logs on the
//              JM proxy to the corresponding TM endpoints.
...
std::optional<Cluster> start_cluster(int n_tms) {
    ...
    for (int i = 1; i <= n_tms; ++i) {
        ...
        const std::string tm_id = "tm-obs-" + std::to_string(i);
...
TEST(HttpObservability, JmMetricsEndpointReturnsPrometheus) {
    auto c = start_cluster(/*n_tms=*/1);
    if (!c.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto r = http_get("127.0.0.1", c->jm_http_port, "/metrics");
    ASSERT_EQ(r.status, 200);
    EXPECT_NE(r.content_type.find("text/plain"), std::string::npos)
        << "content type: " << r.content_type;
    // Prometheus TYPE lines for the process metrics we wired.
    EXPECT_NE(r.body.find("# TYPE " + std::string{metrics::kJmTmsRegistered} + " gauge"),
              std::string::npos)
        << "body: " << r.body;
    EXPECT_NE(r.body.find("# TYPE " + std::string{metrics::kJmJobsSubmittedTotal} + " counter"),
              std::string::npos)
        << "body: " << r.body;
    // Single TM was registered before we hit /metrics.
    EXPECT_NE(r.body.find(std::string{metrics::kJmTmsRegistered} + " 1"), std::string::npos)
        << "body: " << r.body;
}

TEST(HttpObservability, TmMetricsEndpointReturnsPrometheus) {
    auto c = start_cluster(/*n_tms=*/1);
    if (!c.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto r = http_get("127.0.0.1", c->tm_http_ports[0], "/metrics");
    ASSERT_EQ(r.status, 200);
    EXPECT_NE(r.body.find("# TYPE " + std::string{metrics::kTmSlotsCapacity} + " gauge"),
              std::string::npos)
        << "body: " << r.body;
    // Default TM slot count = 4 (set in run_tm via --slots default).
    EXPECT_NE(r.body.find(std::string{metrics::kTmSlotsCapacity} + " 4"), std::string::npos)
        << "body: " << r.body;
}

TEST(HttpObservability, JmLogsEndpointHasRegisterEntry) {
    auto c = start_cluster(/*n_tms=*/2);
    if (!c.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto r = http_get("127.0.0.1", c->jm_http_port, "/api/v1/logs");
    ASSERT_EQ(r.status, 200);
    // JSON envelope.
    EXPECT_NE(r.body.find("\"logs\""), std::string::npos) << "body: " << r.body;
    // Both TM registrations should have produced log lines on the JM.
    EXPECT_NE(r.body.find("\"source\":\"jm.register\""), std::string::npos) << "body: " << r.body;
    EXPECT_NE(r.body.find("tm=" + c->tm_ids[0]), std::string::npos) << "body: " << r.body;
}
...
TEST(HttpObservability, LogsLevelFilterDropsLowerLevels) {
    ...
    // level=error filters out the jm.register (info) lines.
    const auto r = http_get("127.0.0.1", c->jm_http_port, "/api/v1/logs?level=error");
    ASSERT_EQ(r.status, 200);
    EXPECT_EQ(r.body.find("jm.register"), std::string::npos)
        << "info-level register line leaked through error filter; body: " << r.body;
}

TEST(HttpObservability, JmProxiesTmMetricsAndLogs) {
    auto c = start_cluster(/*n_tms=*/1);
    if (!c.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto direct_m = http_get("127.0.0.1", c->tm_http_ports[0], "/metrics");
    const auto proxied_m =
        http_get("127.0.0.1", c->jm_http_port, "/api/v1/tms/" + c->tm_ids[0] + "/metrics");
    ...
    // request to the TM, one is a request to the JM that proxies to the TM -
    // each bumps clink_http_requests_total on the TM). Instead, assert the
    // proxied body still looks like Prometheus exposition.
    EXPECT_NE(proxied_m.body.find("# TYPE " + std::string{metrics::kTmSlotsCapacity}),
              std::string::npos)
        << "proxied body: " << proxied_m.body;

    const auto proxied_l =
        http_get("127.0.0.1", c->jm_http_port, "/api/v1/tms/" + c->tm_ids[0] + "/logs");
    EXPECT_EQ(proxied_l.status, 200);
    EXPECT_NE(proxied_l.body.find("\"logs\""), std::string::npos) << "body: " << proxied_l.body;
}

TEST(HttpObservability, JobSubmissionBumpsJmCounters) {
    ...
    // Baseline: before any submit, jobs_submitted_total = 0.
    {
        const auto r = http_get("127.0.0.1", c->jm_http_port, "/metrics");
        ASSERT_EQ(r.status, 200);
        EXPECT_NE(r.body.find(std::string{metrics::kJmJobsSubmittedTotal} + " 0"),
                  std::string::npos)
    ...
    // Mid-flight: jobs_submitted_total = 1, jobs_running = 1.
    {
        const auto r = http_get("127.0.0.1", c->jm_http_port, "/metrics");
        ASSERT_EQ(r.status, 200);
        EXPECT_NE(r.body.find(std::string{metrics::kJmJobsSubmittedTotal} + " 1"),
                  std::string::npos)
            << "post-submit body: " << r.body;
        EXPECT_NE(r.body.find(std::string{metrics::kJmJobsRunning} + " 1"), std::string::npos)
            << "post-submit body: " << r.body;
    }
