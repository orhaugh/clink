...
TEST(CrossProcessBench, PipelineWallTimeAcrossThreeWorkers) {
    ...
    const auto coordinator_log =
        ...
        ("clink_xpbench_coordinator_" + std::to_string(::getpid()) + ".log");
    ...
    // All 3 by the coordinator's own count. A submission that lands before the
    ...
    ASSERT_TRUE(clink::itest::await_log_matches(coordinator_log, " slots=", 3))
        << "not all 3 workers registered with the coordinator";
