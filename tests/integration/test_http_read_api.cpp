...
//
// Spawns a JM + 2 TMs (all with --http-port set), submits a job, then
...
//   GET /api/v1/cluster           - cluster rollup, embedded TMs
//   GET /api/v1/config            - JM Config snapshot
//   GET /api/v1/jobs              - job list + per-job summary fields
//   GET /api/v1/jobs/:id          - full job detail + 404 path
//   GET /api/v1/tms               - registered TMs
//   GET /api/v1/tm (on a TM)      - TM self-view
//   GET /api/v1/tm/subtasks       - subtask list
//   GET /api/v1/config (on a TM)  - TM Config snapshot
...
// Holder for a JM + 2 TMs all with HTTP ports up. Tests share this
...
struct Cluster {
    ...
    Cluster(Cluster&& other) noexcept
        : jm_pid(other.jm_pid),
          jm_http_port(other.jm_http_port),
          jm_control_port(other.jm_control_port),
          tm_pids(std::move(other.tm_pids)),
          tm_http_ports(std::move(other.tm_http_ports)),
...
std::optional<Cluster> start_cluster(int n_tms) {
    ...
    if (!await_http_ready(c.jm_http_port, 2s)) {
        return std::nullopt;
    }
    ...
    // Give the TMs a moment to register with the JM before tests
...
TEST(HttpReadApi, ClusterEndpointReportsRegisteredTaskManagers) {
    auto cluster = start_cluster(/*n_tms=*/2);
    if (!cluster.has_value()) {
        GTEST_SKIP() << "cluster startup failed (binaries missing or port collision)";
    }
    const auto r = http_get("127.0.0.1", cluster->jm_http_port, "/api/v1/cluster");
    ASSERT_EQ(r.status, 200) << "body: " << r.body;
    EXPECT_NE(r.body.find("\"jobs_total\":0"), std::string::npos);
    EXPECT_NE(r.body.find("\"task_managers\""), std::string::npos);
    EXPECT_NE(r.body.find("tm-read-1"), std::string::npos)
        << "tm-read-1 not visible in /api/v1/cluster: " << r.body;
    EXPECT_NE(r.body.find("tm-read-2"), std::string::npos)
        << "tm-read-2 not visible in /api/v1/cluster: " << r.body;
}

TEST(HttpReadApi, JmConfigEndpointReturnsConfigFields) {
    auto cluster = start_cluster(/*n_tms=*/1);
    if (!cluster.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto r = http_get("127.0.0.1", cluster->jm_http_port, "/api/v1/config");
    ASSERT_EQ(r.status, 200) << "body: " << r.body;
...
TEST(HttpReadApi, JobsEndpointEmptyThenPopulatedAfterSubmission) {
    ...
    {
        const auto r = http_get("127.0.0.1", cluster->jm_http_port, "/api/v1/jobs");
        ASSERT_EQ(r.status, 200) << "body: " << r.body;
    ...
    // it to finish - we just verify the JM tracks it.
    ::setenv("CLINK_CANCEL_TICK_MS", "100", 1);
    const pid_t submit_pid = spawn_proc({"clink_submit_job",
                                         "--job=" + job_so.string(),
                                         "--jm-host=127.0.0.1",
                                         "--jm-port=" + std::to_string(cluster->jm_control_port),
                                         "--wait-timeout-s=20"},
                                        submit);
    ...
    // and the per-job tm placement.
    {
        const auto r = http_get("127.0.0.1", cluster->jm_http_port, "/api/v1/jobs/1");
        ASSERT_EQ(r.status, 200) << "body: " << r.body;
        EXPECT_NE(r.body.find("\"id\":1"), std::string::npos);
        EXPECT_NE(r.body.find("\"tasks\""), std::string::npos);
        EXPECT_NE(r.body.find("tm-read-"), std::string::npos)
            << "expected a tm id under tasks[].tm_id: " << r.body;
    }

    // 404 path.
    {
        const auto r = http_get("127.0.0.1", cluster->jm_http_port, "/api/v1/jobs/9999");
...
TEST(HttpReadApi, TmsEndpointListsRegisteredTaskManagers) {
    auto cluster = start_cluster(/*n_tms=*/2);
    if (!cluster.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto r = http_get("127.0.0.1", cluster->jm_http_port, "/api/v1/tms");
    ASSERT_EQ(r.status, 200);
    EXPECT_NE(r.body.find("tm-read-1"), std::string::npos);
    EXPECT_NE(r.body.find("tm-read-2"), std::string::npos);
...
TEST(HttpReadApi, TmEndpointReportsSelfView) {
    auto cluster = start_cluster(/*n_tms=*/1);
    if (!cluster.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto r = http_get("127.0.0.1", cluster->tm_http_ports[0], "/api/v1/tm");
    ASSERT_EQ(r.status, 200);
    EXPECT_NE(r.body.find("tm-read-1"), std::string::npos);
    EXPECT_NE(r.body.find("\"jm_host\":\"127.0.0.1\""), std::string::npos);
    EXPECT_NE(r.body.find("\"slot_capacity\""), std::string::npos);
}

TEST(HttpReadApi, TmSubtasksEmptyBeforeJob) {
    auto cluster = start_cluster(/*n_tms=*/1);
    if (!cluster.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto r = http_get("127.0.0.1", cluster->tm_http_ports[0], "/api/v1/tm/subtasks");
    ASSERT_EQ(r.status, 200) << "body: " << r.body;
    EXPECT_NE(r.body.find("\"subtasks\":[]"), std::string::npos)
        << "expected empty subtasks before any deploy: " << r.body;
}

TEST(HttpReadApi, TmConfigReturnsSlotCount) {
    auto cluster = start_cluster(/*n_tms=*/1);
    if (!cluster.has_value()) {
        GTEST_SKIP() << "cluster startup failed";
    }
    const auto r = http_get("127.0.0.1", cluster->tm_http_ports[0], "/api/v1/config");
    ASSERT_EQ(r.status, 200);
