...
- Commit subjects: conventional prefix with a scope, lower case after the colon - `feat(cluster):`, `fix(sql):`, `perf(sql):`, `docs(hardening):`, `test(integration):`, `build(image):`, `ci(fuzz):`, `chore(qualification):`. Plain `docs:` and `release: vX.Y.Z` are the accepted unscoped forms. 88% of the history follows this and nothing enforces it, so it is easy to drift by copying the last few subjects rather than the convention - check `git log --format=%s | head -40` before writing one. The body stays as it is: prose explaining why, and what the change is answerable to.
