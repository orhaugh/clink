...
//      to discover where the active JM is.
//
// Epoch: each leadership acquisition bumps a monotonic epoch counter
// written to the leader-endpoint file. Fencing - TMs / messages can be
...
// `advertise_endpoint` is the host:port this JM would publish if it
// became leader (used by the TM's discovery). Pass empty for TMs.
