...
- Exactly-once is provided by snapshot-on-barrier plus two-phase-commit sinks, and applies only to sinks that implement the `on_barrier`/`on_commit` contract; the Parquet and S3 sinks are not two-phase-commit. See [./checkpointing.md](./checkpointing.md).
- The default failure posture is fail-fast: `max_restarts` is 0 and restart-from-checkpoint must be configured (a checkpoint directory and a non-zero restart budget). See [./fault-tolerance-and-rescale.md](./fault-tolerance-and-rescale.md).
