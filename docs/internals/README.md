...
| [Memory budgets](memory-management.md) | Shared tracked-memory budgets, covered state/queue/checkpoint owners, blocking-exchange and SQL aggregate/window/join/OVER/ranking spilling, entry-level aggregate/window/session/OVER/ranking/join state, per-value built-in GROUP BY, fixed-window and session collection state, global top-N and wildcard-entry spilling, retained indexes, failure behaviour and allocation coverage limits. |
...
| [Declared types: fields, derived codec, fingerprints](derived-types.md) | One CLINK_FIELDS declaration per user type: the field-descriptor trait, the derived byte codec and its frozen layout, registration defaults, and the shape fingerprint that gates restores against undeclared field-list changes. |
| [Protocol and format compatibility](protocol-compatibility.md) | The inventory of every independently versioned contract - cluster protocol, data plane, checkpoint sidecar, snapshots, job-plan JSON, connector committables, plugin ABI, embedded C ABI, capture files, capabilities manifest - each with its version, its compatible-change rule, where it is enforced, and the frozen-bytes fixtures that pin it. |
| [Runbook](runbook.md) | What to do when each of the shipped Prometheus alerts fires: what it means, what to look at first, and the likely causes in order - grounded in the failure modes this engine has actually produced. Includes the ones that look like defects and are the machine. |
...
| [The exactly-once specification](exactly-once-specification.md) | The protocol those pages describe, as a TLA+ model that TLC checks in CI: what it states, the invariants it proves over bounded configurations, the fault windows it enumerates, the mutants it is shown to refute, the defects it found, and what it leaves out. |
...
| [Fault tolerance, rescale and schema evolution](fault-tolerance-and-rescale.md) | How clink recovers a job after a lost Worker, rescales operator parallelism via key groups, and migrates keyed state across schema changes, all anchored on completed checkpoints. |
...
| Page | What it covers |
| --- | --- |
| [SQL frontend internals](sql-frontend.md) | How a SQL statement becomes an operator DAG: preparse shim, libpg_query parse, AST, binder, the optimiser (predicate/projection pushdown plus statistics-driven cost-based join reordering, observable through annotated EXPLAIN), and physical planner producing a JobGraphSpec. |
| [Embedded execution](embedded.md) | How `clink run <file>.sql` runs the whole engine in one process with no daemons: the EmbeddedEngine's in-process coordinator + worker pair, the shared SQL script runner, bare-SELECT-to-print, Ctrl-C drain semantics, the collect surface (changelog-aware), the self-contained libclink C ABI, and the Arrow Flight SQL endpoint. |
