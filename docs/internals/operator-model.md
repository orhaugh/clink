...
Most user logic does not implement these interfaces directly. The process-function family in `include/clink/operators/process_function.hpp` (`ProcessFunction<I, O>`, `KeyedProcessFunction<K, I, O>`, `CoProcessFunction`, `KeyedCoProcessFunction`, and the async keyed variants in `async_process_function.hpp` / `async_co_process_function.hpp`) exposes only the record-level hooks (`process_element`, `on_timer`, `flush`) plus `open(RuntimeContext&)` for state, and an adapter operator supplies the stream-element machinery. The fluent Pipeline API builds those adapters itself; a Dag-direct consumer gets them from the public factories `make_process_operator`, `make_keyed_process_operator`, `make_co_process_operator`, `make_keyed_co_process_operator`, `make_async_keyed_process_operator` and `make_async_keyed_co_process_operator`, which deduce the key, input and output types from the function class (`dag.add_operator(h, make_keyed_process_operator(fn, key_fn))`). The adapters live in `namespace detail` and are not part of the Stable surface; the factories are.
...
Operators built by a `DagBuilder` closure - which is every operator on the fluent path, because the closure is registered before the graph exists - never carry a uid themselves, so `derive_id_with_uid_` cannot see one. For those, the uid is stamped onto the runner after construction by `Dag::set_runner_identity(runner_index, spec_node_id, uid)`, which re-derives the runner's `OperatorId` from a non-empty uid. `LocalExecutor` builds each `RuntimeContext` from the runner's id, so that is the id every keyed-state slot is written under. Both build paths call it: `worker.cpp` for a cluster chain, `local_submitter.cpp` for an in-process run. Without that stamp a uid'd operator keeps a position-derived id and its state does not restore - silently, because state that lands under one id and is read under another is indistinguishable from state that was never written.

A value of `0` is reserved (the default-constructed sentinel), so derivation maps a hash of 0 to 1. The DAG rejects duplicate uids (`assigned_uids_`); `set_runner_identity` deliberately does not, since re-stamping the uid an `add_*` path already registered is expected rather than a collision. The same `operator_id_from_uid` transform is used by the fluent front end when it records a job's expected state versions, so both the runtime that stamps state-version metadata and the job's declared expectations agree on keys. The hash is `std::hash<std::string>`, which is only stable within one process build, which is sufficient because the plugin ABI gate pins the same toolchain and commit across the submit and deploy processes. Identity semantics and why a stateful op must set a uid are in [./state-and-backends.md](./state-and-backends.md) and [./fault-tolerance-and-rescale.md](./fault-tolerance-and-rescale.md).
