...
In the SQL frontend (`src/sql/install.cpp`) the production adopters are `AggregateRowOp` (GROUP BY), `EquiJoinRowOp` (stream-stream equi-join), `IntervalJoinRowOp` (interval join, INNER and OUTER - eviction and the OUTER unmatched null-padding run through per-key event-time timers), `DistinctRowOp` (SELECT DISTINCT), `SetOpRowOp` (INTERSECT / EXCEPT), `SemiAntiJoinRowOp` (the plain IN / EXISTS / NOT EXISTS path; null-aware NOT IN keeps the in-memory path because a NULL right poisons probes across keys), and the watermark-driven `WindowRowOp` (TUMBLE/HOP/CUMULATE), `SessionWindowRowOp` (SESSION) and `OverAggregateRowOp` (OVER aggregates). They compute an `effective_async_` flag in `open()` from whether the bound backend defers reads, then return it from `supports_async()`, and their `process_async` issues one `co_await kv.get_async(key)` per record. See ./sql-frontend.md. `DistinctRowOp` holds a presence marker per distinct row in `KeyedState`, so its seen-set is checkpointed (the former in-memory set was not, so a restore replayed already-emitted rows); its state key is built from the same columns the upstream `row_compute_key` hashes, so a row's per-key state lands in the same key-group its record routes to.

The watermark-driven operators (window, session, OVER) do not scan all keys at the watermark - that would not fit per-key async reads. Instead the async path keys per-group/per-partition state in `KeyedState` and registers an **event-time timer** keyed by the group/partition for each buffered window or pending row. A watermark then fires only the due keys' timers (`fires_async_event_time_timers()` -> `on_event_time_timers_async`), so a fire point-reads just that key's state - and distinct due keys' reads coalesce into one `get_many`. `SessionWindowRowOp` registers a timer at each fold's session end + gap + lateness; session merges make those timers dynamic (an extended session leaves a stale timer, a merge leaves duplicates), so the fire re-checks every session's live end against the watermark and fires the due ones - stale and duplicate timers are idempotent no-ops. This sidesteps any "scan the whole state" primitive. `OverAggregateRowOp` covers the whole OVER surface on the async path - running aggregates, FIRST_VALUE / LAST_VALUE, LAG, and bounded ROWS / RANGE frames - because its per-partition `PartState` (running aggregates, the LAG ring, the bounded-frame buffer, the pending buffer) is serialised whole by its codec and `emit_one_` maintains the ring and buffer identically on the sync and async paths. `IntervalJoinRowOp` uses the same timer mechanism for EVICTION rather than firing: each buffered row registers a timer at its eviction point (left `ts + lower + 1`, right `ts + upper + 1`), and the fire drops every row past its window from the due key's two buffers, null-padding an unmatched kept-side (OUTER) row exactly as the sync watermark scan does. The fire takes the largest due timer as the watermark for the cutoff; because a row is due iff its eviction timer is at or below it, that selects precisely the sync eviction set.

A watermark-driven operator that drops late records at ingest needs the watermark *before* the async runner defers its effect to epoch drain. `Operator::on_watermark_observed(wm)` is called synchronously by the single-input async runner the instant it pulls a watermark (before the deferred fire/forward), so e.g. `OverAggregateRowOp` advances its late-drop bound in step with the synchronous path; it is a no-op by default. Such an operator also persists its non-keyed event-time scalars (the late-drop bound) as operator-state in a `snapshot_timers` / `restore_timers` override, and keeps any pending-buffer tiebreaker seq in the per-key `KeyedState` value (not an operator-global counter), so the disaggregated state stays checkpoint-consistent across restore.

### Read coalescing

`coalesce_reads()` is a second opt-in. When true (and async is active), the runner wraps the per-subtask backend in a `CoalescingBackend` (`include/clink/state/coalescing_backend.hpp`) and registers its `flush()` as the controller's flush hook. Each record's `co_await kv.get_async(key)` then lands in the decorator, which does not issue a read immediately: it registers `(op, key)` into a pending batch and suspends the record. When the controller is otherwise stuck (every in-flight record suspended on its read, nothing ready), `drain()` calls the flush hook first, which issues one inner `get_many_async` for the whole pending batch and scatters each result back to its waiting record. So N records reading N distinct keys collapse into one batched (and, on a content-addressed pool, hash-coalesced) round-trip instead of N. This is safe because the per-key gate guarantees every pending read in a batch is for a distinct key, and the flush runs on the runner thread. `AggregateRowOp`, `EquiJoinRowOp`, `IntervalJoinRowOp`, `DistinctRowOp`, `SetOpRowOp` and `SemiAntiJoinRowOp` all set `coalesce_reads()` to true.
...
- **Production adopters.** Among shipped operators the auto-on production adopters of async state are the SQL `GROUP BY` (`AggregateRowOp`), stream-stream equi-join (`EquiJoinRowOp`), and the watermark-driven `WindowRowOp` (TUMBLE/HOP/CUMULATE) and `OverAggregateRowOp` (running-aggregate OVER) via per-key event-time timers. `AsyncLookupOperator` and `HttpPool` are the enrichment path and are not wired to the `AsyncExecutionController`.
