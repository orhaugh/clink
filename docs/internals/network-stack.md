...
Still open: `NetworkBridgeSink` discards `push()`'s `bool`. Chunking removes the oversized-batch trigger, but any other push failure is still silent. A data batch that cannot be admitted should fail its task.
