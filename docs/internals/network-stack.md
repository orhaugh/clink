...
**OPEN DEFECT (found 2026-07-28): a batch larger than the credit window is silently dropped.** Credit is a conserved quantity - the receiver grants back exactly the record count of each batch it pops - so `remaining_credit_` can never exceed the 2048 it was bootstrapped with. `push_remote_` acquires credit for the WHOLE batch and does not chunk, so `acquire_credit_(n)` with `n > 2048` waits on a condition that can never become true. It unblocks only when `closed_` is set at teardown, returns `false`, and `NetworkBridgeSink` discards that `bool`. The batch is gone, with no error and no failed task.
...
Whatever the fix, the silent part has to go first: a data batch that cannot be admitted must fail its task, not vanish. Chunking oversized batches on the send side, or refusing to start a job whose operators can emit batches above the window, are the two candidate shapes.
