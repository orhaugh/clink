...
**Frame cap (fixed 2026-07-28): a batch larger than the credit window used to be silently dropped.** Credit is a conserved quantity - the receiver grants back exactly the record count of each batch it pops - so `remaining_credit_` can never exceed the 2048 it was bootstrapped with. `push_remote_` acquires credit for the WHOLE batch and does not chunk, so `acquire_credit_(n)` with `n > 2048` waits on a condition that can never become true. It unblocks only when `closed_` is set at teardown, returns `false`, and `NetworkBridgeSink` discards that `bool`. The batch is gone, with no error and no failed task.

This is not hypothetical. It is what made nexmark q5 disagree with an independent engine on 153 of 247 windows at parallelism 4 across four workers while matching exactly when every subtask was co-located on one (`benchmarks/nexmark_compare/colocate_q5.sh`). A windowed aggregate fires far bigger batches than a Kafka source delivers - about 4,996 rows per fire there against a source's modest ones - which is why the shuffle in front of the window was unaffected and the one behind it was not, and why the loss is partial rather than total: the 4-way split brings a typical fire to ~1,249 rows per peer, under the window, so only the skewed sub-batches exceed it.

`clink_net_bridge_credit_exhaustion_total` already counts every time the sender blocks on credit, which is the signal that would have surfaced this; nothing was watching it. Also note `kInitialNetworkCredit` is a hard-coded `constexpr` in `wire.hpp` - its own comment says to "tune via JobConfig", and no such knob exists.

Fixed by capping a data frame at `kMaxRecordsPerFrame` (half the credit window, `wire.hpp`): `push_remote_` splits a batch above the window into admissible frames and sends them in order, slicing the Arrow sidecar with `RecordBatch::Slice` so the chunk stays columnar and zero-copy rather than silently downgrading to rows. Half the window rather than all of it, so two frames can be in flight instead of the sender round-tripping per frame. Only batches ABOVE the window are split, so anything that sent successfully before is framed byte-identically. Chunking is semantics-preserving because batch boundaries carry no meaning: watermarks, barriers and drain markers travel as their own `StreamElement`s, and the chunks are emitted in order on one thread with nothing interleaved.

`NetworkChannelCredit.ABatchLargerThanTheCreditWindowArrivesWholeAndInOrder` pins it at sizes straddling the window. It has to force the socket path with `unregister_endpoint` first: `listen()` registers the receive queue with `LocalDataPlane`, so a same-process pair short-circuits and never touches credit - the first version of that test passed with the fix reverted, which is the exact vacuity the defect's own signature ("works co-located, loses records over the wire") should have predicted. Without the fix the test now HANGS rather than fails, because the sender waits on a condition that can never hold; production released it at teardown and dropped the batch instead.
...
**A corrupt receive stream now fails the task too.** The receive side had the same silence in mirror image: the recv loop's error exits (an undecodable Arrow IPC payload, a schema mismatch, a batch parse failure, a truncated or oversized frame, the retired legacy frame kind) were bare returns, so the channel closed, the consumer drained to nullopt, and the task completed as if the stream had simply ended - a single corrupt first frame once took a three-worker pipeline to a successful exit with zero of its records delivered. `NetworkChannelSource::failure_reason()` now goes non-null on every unambiguous-corruption exit (with a `CLINK_NETWORK_CHANNEL_FAILED` stderr line and the recv-error counter), and `NetworkBridgeSource::produce` checks it before reporting end-of-input and throws, routing the loss into ordinary task failure. A clean EOF at a frame boundary remains a normal close: a sender vanishing between frames is indistinguishable from one at this layer, and abrupt peer death is the cluster watchdog's job. Pinned by the `NetworkChannelFailure` tests in `tests/test_network_channel.cpp`.
