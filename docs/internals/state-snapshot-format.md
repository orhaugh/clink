...
retractable `MIN`/`MAX` values with their multiplicities; recovery upgrades both
the whole-partition format and the earlier per-accumulator format. OVER separates
metadata and individual running accumulators from its pending and history lists.
Windows store one pane per entry; sessions store one session per entry. See
