...
| Built-in schemes | `state_backend_factory.cpp` | `memory`, `memory+sharded`, `file`, `changelog`, `changelog+file`, `disagg-local`; plus `rocksdb` / `changelog+rocksdb` (RocksDB impl), `remote-read` (S3 impl), and `forst` / `changelog+forst` (opt-in ForSt impl) |
...
- The factory only registers `rocksdb` / `changelog+rocksdb`, `remote-read`, and `forst` / `changelog+forst` when the respective impls are linked and call `register_scheme` at startup; an unregistered scheme throws at `build()` time.
