...
The five unbounded SQL constructs take the other route: `StateTtlTracker` (`include/clink/sql/state_ttl.hpp`) keeps an in-memory index of absolute per-key deadlines, persisted alongside the data it belongs to, and evicts on each advancing watermark with targeted erases. That is O(expired) rather than O(state), and it is the only shape that works on a deferring backend, where a scan means fetching every value object from the store.
