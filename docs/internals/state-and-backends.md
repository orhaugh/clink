...
**Hiding an entry is not releasing it, and nothing sweeps on your behalf.** Lazy expiry only makes an entry invisible to readers. The memory is reclaimed when `KeyedState::cleanup_batch(budget)` runs, which either scans a bounded slice of the slot (portable backends) or delegates to the backend's own compaction when `supports_expiry_compaction()` is true (RocksDB, ForSt - an LSM rewrites every live SST during compaction, so dropping expired entries there is free). Note that the compaction route is entered *from inside* `cleanup_batch`, so it is not automatic either.

No part of the runtime calls `cleanup_batch` for you, and the SQL runtime no longer calls it at all. An operator written against the typed API must call it itself, periodically - once per checkpoint, or every N records. A job that sets a TTL and never sweeps grows without bound, because every expired-but-unswept entry is still resident and is still written into every snapshot.

**Two things to know before relying on `cleanup_batch` in your own operator.** The budget bounds erasures, not traversal: `StateBackend::scan`'s visitor cannot stop early, so every key under the operator is visited on every call whatever the budget says (and the in-memory backend copies the whole operator's state before visiting it). And the resume cursor lives on the `KeyedState` object, so a caller that rebuilds it per sweep - which is what `RuntimeContext::keyed_state` returning by value encourages - restarts from the front every time and reclaims at most `budget` entries per call however far behind it is. Hold one `KeyedState` across sweeps, and size the budget against the arrival rate rather than against the state.

The five unbounded SQL constructs take the other route: `StateTtlTracker` (`include/clink/sql/state_ttl.hpp`) keeps an in-memory index of absolute per-key deadlines, persisted alongside the data it belongs to, and evicts on each advancing watermark with targeted erases. That is O(expired) rather than O(state), and it is the only shape that works on a deferring backend, where a scan means fetching every value object from the store. Bounded state through this machinery is qualified end to end by [QUAL-05](../qualification/qual-05-ttl-steady-state.md).
...
- **ForStStateBackend** (`forst`, `changelog+forst`; opt-in, `CLINK_WITH_FORST=ON`): keyed state in a [ForSt](https://github.com/ververica/ForSt) instance - an LSM store of RocksDB lineage aimed at disaggregated deployments. Behaviourally a parity peer of the RocksDB backend (same CF-per-operator layout, buffered writes, hard-link incremental checkpoints, `last_snapshot_stats()`, version stamps, `export_arrow_snapshot()` plus the offline `forst_checkpoint_to_arrow(dir)`), verified by a mirrored test suite. Build integration is deliberately isolated: ForSt's CMake still calls itself `rocksdb`, so it builds via ExternalProject at a pinned tag (`impls/forst/CMakeLists.txt`), and it compiles under its own `forstdb` C++ namespace, so one binary can carry both engines - `clink_node` does. Two fetch-time patches keep coexistence sound: the upstream source list hardcodes a JNI-only remote-filesystem bridge (stripped; a native build has no JVM), and two symbol families living outside the engine namespace (`pmull_runtime_flag`, the `XXH*` hash family) are re-prefixed so the two archives' symbol tables are fully disjoint. Upstream's remote-DFS layer is that JNI bridge, so remote storage comes from clink's own seams (`clink::forst_s3`, mirroring `rocksdb_s3`), in two shapes. Checkpoint publication: `s3+forst://` and `changelog+s3+forst://` publish checkpoint dirs / materialization payloads to object storage through the `SnapshotStore` / materialization-store seams, which also flips `supports_async_persist()` on so the durable upload runs on the snapshot worker, off the barrier path. Live remote data files: `s3sst+forst://` runs the engine over a clink-authored filesystem (`impls/forst-s3/src/forst_remote_filesystem.cpp`, on Arrow's S3 filesystem) that keeps the immutable `*.sst` files on object storage - written once via multipart upload, random-read on block-cache miss, checkpoint "hard links" become server-side copies, and the checkpoint's staging-dir rename migrates the objects with it - while the small mutable metadata files (MANIFEST, CURRENT, WAL, LOCK) stay local, so the working set is bounded by object storage rather than local disk. The backend's `DataFileMirror` seam covers the remote half of restore / purge / stats, and the engine's durability point for a remote data file is upload completion at file close - sound here because an SST is fully written and closed before the MANIFEST references it, and job-level durability is clink's checkpoint, not engine crash recovery. Three operational refinements ride the same mapping: checkpoint metadata publishes to the checkpoint's object prefix at persist (via a `SnapshotStore` that also flips the capture/persist split on), so restore needs only the same URI - a fresh machine with an empty disk re-materialises the metadata from the bucket; an optional local read cache (`?sst_cache=` / `?sst_cache_bytes=`) is populated by write-tee as SSTs upload (flush and compaction outputs - the bulk of steady-state reads - serve locally), evicted LRU to its byte budget and dropped when the engine GCs an SST; and stale checkpoint staging left by a crash (`<subtask>.cp-*.tmp` objects - the engine renames its staging dir into place, so a crash strands the remote half) is swept at backend construction. Finally, a deferred-read mode (`Options::defer_reads`; `?defer_reads=1` on `forst://`, DEFAULT ON for `s3sst+forst://` whose block-cache misses are ranged GETs against object storage) makes the backend report `supports_async_get()` and run `get_async` / `get_many_async` on a sized IO executor with per-thread resume-scheduler hand-back (the RemoteReadBackend contract: deadline-aware overload preferred, inline blocking fallback with no runner wired, a batch is one executor job and one suspension). That flips the SQL window / aggregate / join operators onto their async KeyedState paths, so per-record hot state lives in the engine (memtable + SSTs, remote in the s3sst mode) instead of operator-local maps - forst then genuinely carries hot-path state, not just checkpoints. Cross-key emission order becomes completion order (the per-key gate orders only same-key records), the same semantics as every deferring backend.
- **RemoteReadBackend** (`remote-read://...` on S3, `disagg-local://`): the two-tier disaggregated backend, covered in its own section below. Qualified at 29 GiB of live keyed state under sustained fault injection by [QUAL-04](../qualification/qual-04-large-keyed-state.md), which also documents the store-footprint behaviour and the `clink state-sweep` reclamation it calls for.
...
`StateBackendFactory` (`state_backend_factory.hpp`) is a process-wide registry mapping a URI scheme to a builder closure. It consumes a `StateBackendSpec` (working `uri`, `subtask_idx`, `restore_uri`, `restore_checkpoint_id`, and the rescale knobs `restore_from_subtask_idx` / `restore_from_parent_count`) and returns a `BuiltStateBackend` (the `shared_ptr<StateBackend>` plus an optional `restore_from` Snapshot the caller forwards to `JobConfig::restore_from`). A bare path is treated as `file://`; an empty URI is `memory://`. Core pre-registers `memory`, `memory+sharded`, `file`, `changelog`, `changelog+file` and `disagg-local`; the RocksDB impl adds `rocksdb` and `changelog+rocksdb` via `register_scheme` at startup, the S3 impl adds `remote-read`, the rocksdb_s3 composition adds `s3+rocksdb` / `changelog+s3+rocksdb` / `changelog+s3`, and the opt-in ForSt impls add `forst` / `changelog+forst` (base) plus `s3+forst` / `changelog+s3+forst` / `s3sst+forst` (forst_s3 composition). The file builder mints a per-subtask sub-directory (`<base>/<subtask_idx>`) and, on rescale, copies and merges the assigned parents' snapshot files into the new working dir before restore. **Operator-state rows are unioned from every other participating subtask on every restore, not only on a rescale**, and a subtask's own row always wins a key collision. Both halves of that rule are load-bearing. The union is needed because operator-state ownership is not always pinned to a subtask index: a Kafka source subscribes to a consumer group, so the broker decides which subtask owns which partition and that assignment is not stable across a restart - a subtask that comes back holding a partition it never checkpointed would otherwise find no offset for it and silently resume from the broker's committed group offset, replaying or skipping records. Own-wins is needed because not all operator state is partition-scoped: the file, directory, polling and vector sources each store their position under one fixed key, so every subtask writes the same key with a different value, and the merge's keep-the-greater-i64 rule would otherwise hand each subtask the furthest position any of them reached. Both are pinned by tests in `tests/test_state_backend_factory.cpp`.
...
- `queryable_state::Registry` (`registry.hpp`): a process-wide map from slot name to a byte-level `Lookup` closure (`key bytes -> optional value bytes`). `register_slot` / `unregister_slot` / `lookup` / `has_slot` / `slots`, guarded by a shared mutex; the closure is copied out before it runs so it can take its own locks. `Registry::global()` is the instance the Worker's routes serve; operators bind into it.
- `bind_keyed_state` (`bind.hpp`): wires a `KeyedState<K,V>` into a registry slot; the closure decodes the inbound key bytes via the K codec, hits the slot through the operator's encode-key path, and encodes the value back via the V codec. `bind_keyed_state_for_subtask` composes a `(role, subtask_idx, slot)` triple so subtasks of one op on the same worker do not collide.
- `register_routes` (`server.hpp`): adds worker HTTP routes. `GET /api/v1/queryable_state` lists byte and JSON slots; `GET /api/v1/queryable_state/:slot?key=<hex>` looks one up; a subtask-scoped route mirrors it under `/op/:role/subtask/:subtask/slot/:slot`. Key and value bytes are lowercase-hex on the wire.
- `register_coordinator_routes` (`coordinator_routes.hpp`): coordinator-side discovery and key routing. `/job/:job_id/workers` lists the workers hosting the job; `/job/:job_id/op/:role/route?key=<hex>` computes `key_group_for_key` and returns the single subtask responsible (host, port, subtask_idx) plus a `topology_version`; `/job/:job_id/topology_version` is a cheap poll clients use to detect rescales and invalidate cached routes.
...
- The SQL `aggregate_row` operator (unbounded GROUP BY, in-memory path) **binds automatically**: at `open()` it registers a JSON slot at `(deployment role, global subtask, "agg")` when the runner identity is present, and unregisters at `close()`. The served value is the key's current FINALISED output row (group columns + aggregate results) - what a downstream consumer would see - built on demand from the live bucket. Lookups run on the worker's HTTP thread against operator-thread state, so the fold paths and the closure share a serving mutex the hot path takes once per batch (not per record). Keys are the operator's composite group-key string; for the common single-TEXT-key GROUP BY the closure also accepts the bare string (it retries it JSON-quoted).
- Worker route: `GET /api/v1/queryable_state/op/:role/subtask/:subtask/json/:slot?key=<string>` returns `{"key":"...","value":{...}}`.
- Coordinator one-call serving route: `GET /api/v1/queryable_state/job/:job_id/op/:role/json/:slot?key=<string>` fans the lookup out across the role's subtasks (in subtask order) and relays the first hit - correct at any parallelism without reproducing the shuffle's key hashing coordinator-side; the key-group `/route` fast path remains for latency-sensitive clients. Keys must be URL-safe.
- Scans (state-as-table): the registry's `JsonScan` closures return up to `limit` (key, value) entries plus a `truncated` flag. The worker route `.../json/:slot/scan?limit=N` (default 1000, clamped to 100000 - a scan holds the operator's serving lock for its duration, so reads are bounded snapshots, not cursors); the coordinator's `.../op/:role/json/:slot/scan` merges every subtask's scan in subtask order. The SQL source `connector='queryable_state'` reads the coordinator scan route and emits one Row per key, so one job can SELECT from (or join against) another job's live state - see [the built-in connectors page](../connectors/builtin.md).
- Arrow bulk reads (`arrow_scan.hpp`): the same scans as ONE Arrow IPC stream over plain HTTP - `.../scan.arrow?limit=N` on both the worker (subtask-scoped) and the coordinator (fanned out and merged). The body is directly readable by pyarrow / polars / duckdb (`Content-Type: application/vnd.apache.arrow.stream`; truncation rides the `X-Clink-Truncated` header since the body is binary). Schema: `__key` utf8 plus the value documents' fields, kinds inferred from the first entry (number -> float64 - JSON numbers are doubles - bool -> bool, string -> utf8, anything else -> utf8 of the serialised JSON; a missing or differently-kinded field in a later row becomes null). Deliberately NOT Arrow Flight: serving state adds no gRPC dependency. The routes register via `register_arrow_scan_route` / `register_coordinator_arrow_scan_route`, kept out of `server.hpp`/`coordinator_routes.hpp` so only Arrow-serving binaries pay for the Arrow includes.
- Live whole-job export (`live_export.hpp`): `GET /api/v1/state/export/job/:job_id` on the worker merges every hosted subtask backend's live Arrow export for the job into one canonical state-snapshot stream (`Worker::export_job_state_arrow`, served off the checkpoint-registered `per_job_backends_`); the same path on the coordinator fans out across every worker hosting the job (`workers_hosting_job`) and merges their streams. Per-subtask atomic, NOT a checkpoint-consistent global cut (no barrier alignment) - use a savepoint where cross-subtask consistency matters. The pool-backed `RemoteReadBackend` exports its COMPLETE live view - the last durably-committed pool checkpoint enumerated via `RemotePool::scan_checkpoint` (implemented by the in-memory and S3 pools; one object GET per distinct content hash on S3), overlaid with the hot tier's dirty writes and masked by erased keys, mirroring `get()`'s precedence exactly. Only the loader-only construction refuses (no enumerable durable tier); refusals are counted in the worker response's `X-Clink-Skipped-Subtasks` header rather than silently omitted. `clink state-export --job=<id> [--coordinator=host:port]` and `clink state-query --job/--coordinator` consume the coordinator route.
...
| `queryable_state::Registry` / `register_routes` / `register_coordinator_routes` / `bind_keyed_state` | External read access and coordinator key routing |
...
| Built-in schemes | `state_backend_factory.cpp` | `memory`, `memory+sharded`, `file`, `changelog`, `changelog+file`, `disagg-local`; plus `rocksdb` / `changelog+rocksdb` (RocksDB impl), `remote-read` (S3 impl), and `forst` / `changelog+forst` / `s3sst+forst` (opt-in ForSt impls) |
...
- The factory only registers `rocksdb` / `changelog+rocksdb`, `remote-read`, and the `forst` family when the respective impls are linked and call `register_scheme` at startup; an unregistered scheme throws at `build()` time.
...
GROUP BY also splits its built-in distinct, retractable, percentile, string and
array collections into individual value cells. Windows split panes and sessions;
join buffers split rows. Null-aware joins also store exact-key probes and cross-key
