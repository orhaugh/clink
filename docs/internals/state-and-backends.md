...
No part of the runtime calls `cleanup_batch` for you. The SQL runtime calls it on its own keyed operators; an operator written against the typed API must call it itself, periodically - once per checkpoint, or every N records. A job that sets a TTL and never sweeps grows without bound, because every expired-but-unswept entry is still resident and is still written into every snapshot.
