...
- The SQL window operators are durable on a synchronous backend. `WindowRowOp` and `SessionWindowRowOp` keep their hot state in in-memory maps unless the backend defers reads, and flush it to the `win` slot in `snapshot_timers` (the runner's pre-snapshot hook), reloading it in `open()` - the same shape `AggregateRowOp` uses for `agg`. Before 2026-07-28 the window operators did not flush, so a restore brought back an operator with no open windows: the source resumed at the checkpointed offset, the records already folded into those windows were never replayed, and their contribution was silently lost.
