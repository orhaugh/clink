...
- The three watermark strategies are exactly those shipped (monotonic, bounded out-of-orderness, partition-aware bounded). The partition-aware strategy's quiet-partition limitation (a partition that stops producing pins the min low) is documented in the source; a per-partition idleness timeout is a noted follow-on.
...
- The SQL windowed-aggregate operators (`WindowRowOp` for `TUMBLE`/`HOP`/`CUMULATE`, in `src/sql/install.cpp`) are a separate, thinner implementation from the typed operators above. They now **drop late records**: a record whose window has already fired and been purged (window end at or under the current watermark, less `allowed_lateness_ms_` which defaults to 0) is dropped instead of re-creating the window and emitting a duplicate, partial row. The drop horizon is captured at ingest on every path - sync, columnar, and the async/disaggregated coroutine path (where the fold is deferred, so the ingest-time watermark, not the fold-time one, is the correct cutoff). Still on the SQL path as follow-ups: a SQL knob to raise `allowed_lateness` (with late re-fire), a late side output, the same drop guard for `SessionWindowRowOp`, and idle-source watermark handling.
