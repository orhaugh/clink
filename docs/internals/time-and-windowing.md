...
- Tumbling: one window, `start = floor(ts / size) * size`.
...
Every kind computes that assignment through one shared header, `include/clink/time/window_arithmetic.hpp` (`floor_div`, `window_start_for`, `window_end_for`, `hop_window_starts_for`, `sat_add`, `sat_sub`). It is shared because the same two lines had been written independently at ten sites, and nine of them used C++ integer division, which truncates toward zero rather than flooring. The two agree for every non-negative timestamp, so the divergence only showed up before the Unix epoch: a record at `ts = -15000` in a 10s tumbling window landed in `[-10000, 0)`, which does not contain it, and a hopping window - which additionally clamped its first pane to zero - discarded such records outright. The helpers also compute every intermediate in 128 bits and clamp once, because `start + size`, a floored start near `INT64_MIN`, and a hop's `ts - size + slide` all overflow at the extremes of `int64` - and `INT64_MIN` / `INT64_MAX` are exactly what an uninitialised or sentinel timestamp column carries. Saturation leaves the topmost window closed at `INT64_MAX` rather than half-open, since no larger bound is representable.
...
Beyond windows, operators can register event-time and processing-time timers through `RuntimeContext::timer_service()` (`register_event_time_timer(ts, key)` / `register_processing_time_timer(ts, key)`). The base `Operator::on_watermark` default fires every event-time timer whose timestamp is at most the watermark via `fire_due_event_time_timers`, then forwards the watermark.

Because the runner drives that call on every watermark it forwards, whether or not a timer is due, the `TimerService` is also where event-time progress is recorded: `TimerService::current_watermark()` returns the highest watermark the operator has been driven with, surfaced to operator code as `ProcessFunctionContext::current_watermark()` and `AsyncKeyedProcessContext::current_watermark()`. An operator that keeps its own ordering - a reordering buffer, a custom join, anything that must decide whether a record can still be placed against records yet to come - needs to read it: at or below the watermark, it cannot. The window operators express the same judgement as `allowed_lateness` plus a late output tag. It is not part of the timer snapshot, so after a restore it is empty until the next watermark arrives; an operator whose correctness depends on it across a restore should keep its own high-water mark in keyed state. Note also that watermarks are emitted per source batch rather than per record, so the granularity at which the question can be answered is one batch of event time. Window operators override `on_watermark` and own their firing directly rather than relying on per-window timers; the assigner's idleness probe uses a processing-time timer.
...
- The SQL windowed-aggregate operators (`WindowRowOp` for `TUMBLE`/`HOP`/`CUMULATE` and `SessionWindowRowOp`, in `src/sql/install.cpp`) are a separate, thinner implementation from the typed operators above. They now **drop late records**: a record whose window has already fired and been purged is dropped instead of re-creating the window and emitting a duplicate, partial row. For `WindowRowOp` the cutoff is "window end at or under the current watermark (less `allowed_lateness_ms_`, default 0)", captured at ingest on every path - sync, columnar, and the async/disaggregated coroutine path (where the fold is deferred, so the ingest-time watermark, not the fold-time one, is the correct cutoff). For `SessionWindowRowOp` (sync + columnar; no async path) the guard is applied only when a record would OPEN a fresh session: if that single-event session is already purged (`ts + gap` at/under the cutoff) it is dropped, whereas a record that MERGES into a still-live session legitimately extends it and always proceeds. Idle-source handling is wired through SQL via the source-table `idle_timeout_ms` option (per-partition + stream-level idleness on the assigner).
- **Allowed lateness (fire-once grace band) and the late side output** are wired on the SQL window path via two source-table options, threaded to the window op by the planner (`first_source_property` in `src/sql/physical_plan.cpp`):
  - `allowed_lateness_ms` (default 0) holds a fired window/session open for a grace band: the fire/purge threshold becomes `window_end + allowed_lateness_ms` (sessions: `end + gap + allowed_lateness_ms`), so a late-but-within-band record still folds into the window before its single emit. The semantics are **fire-once after the grace band**, not re-fire-per-late-record: the window emits exactly one row when the watermark passes the band, with every within-band record already included. This keeps the codec and all three `WindowRowOp` paths (sync, columnar-WS3, async) byte-identical - the async path registers its event-time timer at `window_end + allowed_lateness_ms` and maps it back to `window_end` on fire. The cutoff is still frozen at ingest (`current_wm_ - allowed_lateness_ms`).
  - `late_records_to_dlq` (default off) routes a record dropped as fully-late to the ambient dead-letter channel (`RuntimeContext::report_bad_record`, `connector = sql_window` / `sql_session_window`, `direction = operator`) instead of dropping it silently. Best-effort and not barrier-tied (a replayed late record is re-reported), consistent with the DLQ contract. The drop is detected where the fold sees zero live windows for the record (`WindowRowOp::fold_record_into_` folded-count, plus the async pre-filter short-circuit) and, for sessions, on the new-session branch. **Gap:** the fully-vectorised WS6 columnar session fold (`fold_session_columnar_`) drops late records silently - it has no per-record `Row` to hand the DLQ - so `late_records_to_dlq` does not cover that one path.
- The SQL window operators read the event-time column exactly (`event_time_of`), not through a `double`. A `double` carries 53 bits of mantissa, so a timestamp above about 9.0e15 was previously rounded before window assignment: epoch **milliseconds** (~1.7e12) survive that, epoch **nanoseconds** (~1.7e18) do not.
- The SQL window operators are durable on a synchronous backend. `WindowRowOp` and `SessionWindowRowOp` keep their hot state in operator-owned maps unless the backend defers reads. With a shared budget and `CLINK_SQL_SPILL_DIR`, those maps use per-pane or per-session disk entries; OVER separates pending rows, history and running accumulators. The operators flush state in `snapshot_timers` (the runner's pre-snapshot hook) to the same slot their async paths use - `win` for the fixed windows, `session` for sessions - reloading it in `open()`. This is the shape `AggregateRowOp` uses for `agg`. Before 2026-07-28 neither flushed, so a restore brought back an operator with no open windows: the source resumed at the checkpointed offset, the records already folded into those windows were never replayed, and their contribution was silently lost. `WindowRowOp` was fixed first, `SessionWindowRowOp` a day later, and `TopNPerKeyRowOp` (the `ROW_NUMBER` top-N / dedup changelog operator, slot `topn`) the day after that - three separate discoveries of one defect class, each fix written against one class while its siblings stayed broken. For the changelog operator the symptom is sharper than lost output: restored empty, a displacing arrival emits no delete for the row it displaces, so the stale row stays live in an upsert sink forever. Its codec also round-trips each partition's vector ORDER, because `ROW_NUMBER` ties break by arrival order and that order exists nowhere but the vector; `TopNStateLifecycle` in `tests/test_sql_runtime.cpp` pins both properties. An inventory of every stateful Row-channel operator now lives with the pending-gaps list: joins, distinct and set-ops hold state per-record in `KeyedState` (durable by construction on a synchronous backend). `LastNAggRowOp` (bounded `ROWS` OVER frames, slot `lastn`) joined the durable family on 2026-07-29 - the fifth member, found by that inventory rather than by a production surprise; its codec serialises both the frame window (order included - eviction pops the front) and `prior_emitted`, the row its changelog last declared live, without which a drained window emits nothing instead of its delete. `LastNAggStateLifecycle` pins both. `OverAggregateRowOp`'s sync path - the inventory's last unverified entry - was then VERIFIED BY TEST to have the same defect (a pending pre-checkpoint row lost outright, every post-restore running value computed from an empty fold) and fixed the same day, closing the family at six: it flushes its sync partitions to the `over` slot the async path already checkpoints through, and persists the sync tie-break counter in the `over.wm` blob (9 -> 17 bytes, trailing-bytes-tolerant; an old blob falls back to a hydrate-time max over restored pending seqs) so a restarted-to-zero counter cannot collide a new `(ts, seq)` key with a restored one - `map::emplace` on a duplicate silently drops the record. `OverAggStateLifecycle` pins both hazards. Unlike TopN/LastN there is no deferring-backend gap here: on a deferring backend the async path owns this state per-record. The two file-materialising changelog SINKS (`FileJsonUpsertSink`, slot `upsert`; `ChangelogNetSink`, slot `net`) followed on 2026-07-29: both write the file only at end-of-stream, so mid-job the whole compacted view lived in memory and a restore lost every row whose last change predated the checkpoint - a data-LOSS class, not the duplicate-emission class the gap ledger first recorded it as. Their persistence is WRITE-THROUGH at `on_data` (dirty keys coalesced per batch), deliberately not `on_barrier`: the sink runner snapshots the backend BEFORE delivering the barrier to the sink, so a barrier-time flush would be one checkpoint stale, forever. Deletes write through too - `UpsertSinkLifecycle`'s double-failover test resurrects a zombie row if they do not - and the netting sink round-trips its MULTIPLICITY, since a count of 2 restored as less nets to empty one delete early.
- **Memory per (key, window).** The unit to reason about is the pane, not the group: a `HOP` of size `S` and slide `s` holds `ceil(S / s)` buckets per key (a 10s/2s hop, five) and a `CUMULATE` holds `S / step`. Measured 2026-07-28 for the cheapest realistic shape, an `int64` group key and a `COUNT(*)`:

  | | serialised, per (key, window) | what it is |
  |---|---|---|
  | `TUMBLE` | 127 B | `window_start`, the group values as a JSON object string, and one encoded `AggState` |
  | `HOP` | 116 B | as above, stored once per overlapping pane |
...
- The inline fluent CEP `.select` / `.flat_select` capture the user function into the process-wide runner registry, so submitting such a job to a remote Worker that lacks the registration fails there; package the operator as a real plugin for cross-process CEP jobs.
...
SQL fixed/session windows and OVER frame state share the operator memory budget.
Configured scratch spill preserves pane/session boundaries, pending row order
and folded history; watermark scans read individual entries. One pane/session
aggregate payload, individual row or growing accumulator must still fit. See [Memory budgets](memory-management.md).
