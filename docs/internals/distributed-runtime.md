...
A clink cluster has exactly one active Coordinator (coordinator) and any number of Workers (workers). The coordinator is the single source of truth for deployment: it accepts job submissions from clients, plans them into per-subtask deployment tasks, hands those tasks to workers, tracks liveness and checkpoint progress, and reports completion. Each worker is a worker that registers with the coordinator, receives deployment tasks, runs them, and reports back. All three roles (coordinator, worker, client) speak one binary, length-prefixed framing defined in `include/clink/cluster/protocol.hpp`. This control plane is qualified at width by [QUAL-06](../qualification/qual-06-dag-scaling.md): a 147-operator SQL graph deployed as 292 network-bridged subtasks, exactly once under sustained fault injection, recovering a worker kill at that width in 28 seconds.

Jobs are not built into the cluster binaries. A job is a compiled shared library that the client ships with the submission; the coordinator caches it and re-ships it inside every deployment, and each worker `dlopen`s it before running tasks. This is the job-as-plugin model (`CLINK_REGISTER_JOB`). An out-of-tree module builds with `clink_add_job_module()` from the installed CMake package (`docs/consumer-examples/CMakeLists.txt` shows the shape), the same recipe the repository's own job modules and the ABI gate's fixtures use; see [design record 010](../design/010-stable-extension-model.md) for the compatibility contract it carries.
...
The lease's health depends on the reader thread never blocking, so the worker executes sink commit callbacks on a dedicated FIFO dispatch thread rather than on the reader. A 2PC commit blocks on the external system - a Kafka `commit_transaction` against an unreachable broker holds for the full transaction timeout - and when that ran on the reader, no frame was read and the coordinator-contact clock froze for the duration: a broker outage then read as coordinator silence, and perfectly healthy workers ended their own sessions mid-outage, converting a data-plane fault into control-plane worker loss and restart storms. The single dispatch consumer preserves per-worker commit order across checkpoints, which the transactional sinks' open-transaction sequencing relies on; retirement gates refuse dispatches that outlive their runner exactly as before.
...
`PluginLoader` (`src/cluster/plugin_loader.cpp`) `dlopen`s a `.so` with `RTLD_NOW | RTLD_LOCAL`, resolves the extern "C" handshake symbols (`clink_plugin_abi_fingerprint`, `clink_plugin_abi_version`, `clink_plugin_abi_hash`, `clink_plugin_target_triple`, `clink_plugin_toolchain`, `clink_plugin_abi_manifest`, `clink_plugin_metadata`, `clink_plugin_register`), and gates the load on three checks, all folded into the pure `check_plugin_abi()` for testing (design record [010](../design/010-stable-extension-model.md)):

- **ABI compatibility.** The default gate compares `clink_plugin_abi_fingerprint()` to `cluster_abi_fingerprint()` - both return `kAbiFingerprint`, a **structural fingerprint** computed at clink configure time (`CMakeLists.txt`) as `SHA256` over the content of the **declared extension surface**: the headers named by the tracked manifest `scripts/plugin-abi-surface.txt`, generated by `scripts/gen-plugin-abi-surface.py` as the transitive include closure of the plugin authoring entry points (`include/clink/{api,job,plugin,operators,connectors}/`), plus the conditional-compilation options that surface uses (the manifest's `[options]` section), the pinned Arrow version and the manual `CLINK_ABI_VERSION` - baked into the generated `include/clink/plugin/abi_version.hpp`. Equal fingerprints load; a difference is a hard refusal, and because every plugin also carries its per-header manifest (`clink_plugin_abi_manifest`), the refusal **names the headers that differ** (or blames the options material when the manifests are identical). It rotates when the surface a plugin can reach across the dlopen boundary actually changes - a boundary-type layout, a virtual on `Operator`/`Source`/`Sink`, an inline/template body, a toggled ABI option - but **not** on host-side header edits (`sql/`, `embed/`, `queryable_state/`, ...) or `.cpp` / test / doc / build-only commits. A header entering or leaving the closure is a reviewed manifest diff, gated by `--check` in CI and the pre-commit hook, and the generator fails if any first-party plugin source includes a header outside the declared surface. Two fallbacks preserve safety: a plugin built before the fingerprint symbol existed and **strict mode** (`CLINK_STRICT_PLUGIN_ABI=1`) both revert to the historic exact commit-hash comparison (`kAbiHash`, from `git rev-parse HEAD`). The commit hash is otherwise informational (reported in logs, `LoadedPlugin`, and `clink_node --version`).
...
- **Toolchain identity.** `kToolchain` (in the generated header) is composed by the **preprocessor** per translation unit - which standard library, the libstdc++ dual-ABI choice, `_GLIBCXX_DEBUG`, and ASan/TSan/MSan instrumentation - so a module baked under one toolchain cannot load into a cluster built under another (a TSan cluster refuses an uninstrumented plugin, a libc++ host refuses a libstdc++ module). Standard-library *versions* are deliberately not part of the string: both mainstream libraries hold their ABI stable across releases. A plugin without the symbol predates the fingerprint that introduced it and is already refused by the ABI gate, so the absent-symbol case waves nothing through.

Deploy-time refusals by any of the three gates are **fatal** (`SubtaskFinishedMsg.fatal`): they are deterministic for the binaries involved, so the coordinator fails the job with the cause instead of consuming restart budget on redeploy-and-refuse loops. They are also reachable only on a mixed-version cluster - submit-time checks make same-version refusals happen earlier:

- **Submit preflight.** The submitter reads each plugin's identity via its own `dlopen` and advertises it inside `SubmitJob` (`PluginAbiAdvert`, an eof-guarded tail). The coordinator checks adverts against its own identity **before resolving references or requesting bytes**, so an incompatible plugin is refused on the references-only first exchange with zero plugin bytes shipped; the ack carries the cluster's identity and (on refusal) its manifest, and the client names the differing headers against its local copy. Adverts are best-effort claims - the load-time gate remains the authority.
...
- **ABI gate is a structural fingerprint over a declared surface.** By default a plugin and the cluster must agree on a SHA-256 over the declared extension surface (the headers in `scripts/plugin-abi-surface.txt` plus the build options that surface uses, the pinned Arrow version and `CLINK_ABI_VERSION`), on the target triple, and on the toolchain identity (stdlib, dual-ABI choice, sanitizer instrumentation) - so a cluster rebuild keeps existing plugin binaries deployable unless the contract genuinely moved, and a refusal names the differing headers. Incompatible submits are refused on the references-only exchange before any bytes ship; deploy-time gate refusals are fatal rather than retried. `CLINK_STRICT_PLUGIN_ABI=1` restores the legacy exact-commit-hash comparison. There is no sandbox: a crash in plugin code terminates the worker process, after which the coordinator restarts the job per its restart policy (the contract is "trust your own plugins"). See design record [010](../design/010-stable-extension-model.md).
