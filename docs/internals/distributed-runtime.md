...
A `--ha-dir` TM in `clink_node` discovers the leader by polling `active-leader.json` (or etcd) for up to 10 s, connects to it, and exits non-zero on JM disconnect so an external supervisor can restart it; the restart re-reads `active-leader.json` to find the new leader. TMs can also discover the JM endpoint through the simpler `ServiceDiscovery` abstraction (`include/clink/cluster/service_discovery.hpp`): static config, environment variables, or a file containing `host:port`.
...
- in each **TM** process, `dlopen` fires the same `build_fn` (call-once is per process), and the side-effect registrations populate that process's runner registry, so inline operator types such as `_inline_map_<n>` resolve identically on both sides.

The same `.so` is therefore dlopen'd on every process that touches the job: the submitter, the JM (for planning and validation), and each TM (to run it). Cross-process matching of inline operator types relies on `build_fn` registering operators in a deterministic order.

#### Plugin loading and the ABI gate

`PluginLoader` (`src/cluster/plugin_loader.cpp`) `dlopen`s a `.so` with `RTLD_NOW | RTLD_LOCAL`, resolves the extern "C" handshake symbols (`clink_plugin_abi_version`, `clink_plugin_abi_hash`, `clink_plugin_target_triple`, `clink_plugin_metadata`, `clink_plugin_register`), and gates the load on two checks:

- **ABI compatibility.** The default gate is `clink_plugin_abi_version()` vs `cluster_abi_version()` - both return `kAbiVersion`, a single integer baked into the generated `include/clink/plugin/abi_version.hpp` from `CLINK_ABI_VERSION` in `CMakeLists.txt`. Equal versions load; a difference is a hard refusal. `kAbiVersion` is **bumped by hand only on an intentional ABI break** - a layout change that crosses the dlopen boundary (a data member added to `RuntimeContext`/`BuildContext`/`StreamElement`/`Batch`/`Record`/`PluginRegistry`, a virtual added or reordered on `Operator`/`Source`/`Sink`, an enum's underlying type, an inline/template body compiled into plugins, or the `CLINK_USE_FLAT_HASH_MAP` `FlatMap` choice). So a plugin survives any marketing-version bump (patch or feature) that leaves the ABI intact, and only a deliberate break invalidates it. The decision is factored into the pure `check_plugin_abi()` for testing. Two fallbacks preserve safety: a plugin built before the version symbol existed (`clink_plugin_abi_version` absent) and **strict mode** (`CLINK_STRICT_PLUGIN_ABI=1`) both revert to the historic exact commit-hash comparison (`kAbiHash`, from `git rev-parse HEAD`). The commit hash is otherwise informational (reported in logs, `LoadedPlugin`, and `clink_node --version`).
- **Target triple.** The plugin's target triple (`darwin-arm64` / `linux-x86_64` / `linux-arm64`) must match the cluster's, or the load is refused. This gate is unconditional - it is a genuine binary-compat axis independent of ABI version.
