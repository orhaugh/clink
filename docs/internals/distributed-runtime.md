...
`PluginLoader` (`src/cluster/plugin_loader.cpp`) `dlopen`s a `.so` with `RTLD_NOW | RTLD_LOCAL`, resolves the four extern "C" handshake symbols (`clink_plugin_abi_hash`, `clink_plugin_target_triple`, `clink_plugin_metadata`, `clink_plugin_register`), and gates the load on two byte-for-byte checks:

- **ABI hash.** The plugin's `clink_plugin_abi_hash()` returns `kAbiHash`, a constant baked into the generated `include/clink/plugin/abi_version.hpp` from `git rev-parse HEAD` at configure time. The cluster compares it to its own `cluster_abi_hash()` with `strcmp`; a mismatch is a hard refusal. The plugin and cluster must be built from the same clink commit, which is what makes passing STL types (`std::string`, `std::function`, `std::shared_ptr`, codecs) across the dlopen boundary safe.
- **Target triple.** The plugin's target triple must match the cluster's, or the load is refused.
