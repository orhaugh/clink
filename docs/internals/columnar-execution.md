...
| Generic user-type batcher | `include/clink/core/columnar_batcher.hpp` | `CLINK_ARROW_FIELDS` + `make_columnar_arrow_batcher<T>()` for plain structs |
...
2. The generic struct batcher, `make_columnar_arrow_batcher<T>()` in `columnar_batcher.hpp`. A user describes a plain struct once at namespace scope with `CLINK_ARROW_FIELDS(Trade, id, symbol, px)`; the generator folds over that field list at compile time to synthesise schema/build/parse, emitting one typed Arrow column per field (`{event_time, id:int64, symbol:utf8, px:float64}`). Field types are mapped by `ArrowColumnTraits` (fixed-width integers 8 to 64 bit signed and unsigned, `float`, `double`, `bool`, `std::string`); an unmapped type is a readable compile error. This makes the wire and Parquet layout genuinely columnar and externally typed; it does *not* auto-vectorise operators (an operator that understands the schema is still bespoke).
