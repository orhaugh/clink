...
3. **Drain -> rebind -> deploy.** The old subtasks' exits are counted as drained acks (translated from the shared role to the operator through the deploy-time identity records). An exit can reach the coordinator before C's completion does - the old subtasks stop the moment they forward C, while C completes on the last participant's ack, the sink's - so an exit that arrives first is held on the cutover and counted the moment C completes (or released to the replan if the cutover aborts first); the order between the two never decides whether the cut finishes. The fed tasks then bind one new inbound listener per post-cutover subtask (`CutoverRebind`) and report the ports via mid-run `SubtaskListening`s - the same shape as the at-deploy report, so peer resolution is unchanged. The validated plan deploys the new subtasks: job-global indices *appended* past the deployed allocation (append-only - no other task's index, key-group range or state directory changes, and nothing else redeploys), restoring from C in the same state generation, each mapped onto its parents' deployed directories by the same `rescale_parent_mapping` arithmetic the replan uses.
...
`check_restore_compatibility` returns a list of `(op, state_type, from, to)` pairs the registry cannot bridge; an empty list means the restore is compatible. The coordinator calls `check_restore_compatibility_via_plugins` at job submission: it reads subtask 0's stored stamps and asks each job `.so` (where the job's `StateMigrationRegistry` lives) whether it can migrate to its expected versions, rejecting the deploy on a definite incompatibility. The same gate then runs the SHAPE check when both sides can: a job that declared slot shapes via `expect_state_shape<V>()` exports `clink_job_check_restore_fingerprints`, and a stored fingerprint differing from the declared one with no version bump covering the slot (`check_fingerprint_compatibility`, the pure pre-deploy twin of the bind-time gate) rejects the submit naming the slot and both fingerprints - "the bump nobody made", caught before any deploy. A `.so` predating the export, an undeclared job, or a stampless savepoint is simply not fingerprint-gated; the bind-time gate remains the restore-time backstop. This gate is best-effort by design: if it cannot read the savepoint, load a `.so`, or find the exported check, it returns empty and lets the deploy proceed, since `migrate_restored_state` still guarantees correctness (or throws) at restore. The offline `clink check-savepoint` CLI exposes the same inspection for operators without a running coordinator, printing both stamp tables and checking both dimensions under `--expected`. This machinery is what carries a rolling engine upgrade: [QUAL-08](../qualification/qual-08-rolling-upgrade.md) qualified a real revision pair end to end - savepoint in 2 seconds, restore on the new engine in 2 seconds, 114 seconds of downtime, and exactly-once accounting across the whole boundary under the fault battery.
