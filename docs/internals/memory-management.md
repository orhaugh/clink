...
| --- | --- |
| GROUP BY | Each aggregate accumulator is separate from group values and prior changelog output. `COUNT(DISTINCT)` and retractable `MIN`/`MAX` split their value collections into individual multiplicity cells, which updates and finalisation scan one at a time. Other updates and queryable lookups finalise one accumulator at a time; TTL erases both accumulator and value cells. |
...
`STRING_AGG`, `ARRAY_AGG`, percentile state and opaque UDAFs remain whole
accumulators. Their materialised results must fit too. Codec buffers,
input/output batches, TTL indexes and backend caches retain the allocation
limitations above.
