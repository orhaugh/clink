...
| Memory budgets | Opt-in shared accounting for local queues, covered SQL aggregate/window/join/OVER/ranking state, memory/file backends and checkpoint buffers. Blocking exchanges and covered synchronous SQL partitions spill on pressure when configured. Global top-N and null wildcard entries can also spill; TTL indexes stay in RAM. Entry-level GROUP BY, window/session, OVER, ranking and join storage supports oversized active keys; `COUNT(DISTINCT)`, retractable `MIN`/`MAX` and percentile aggregates split their value collections too. Individual rows, pane/session aggregate payloads, materialised results and other growing accumulators must still fit. Partial allocation coverage, not an RSS cap; cluster limits are per local execution | [Memory budgets](internals/memory-management.md) |
