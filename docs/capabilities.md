...
| Memory budgets | Opt-in shared accounting for local queues, covered SQL aggregate/window/join/OVER/ranking state, memory/file backends and checkpoint buffers. Blocking exchanges and covered synchronous SQL partitions spill on pressure when configured. TTL and null wildcard indexes and global top-N stay budgeted in RAM. Partial allocation coverage, not an RSS cap; cluster limits are per local execution | [Memory budgets](internals/memory-management.md) |
