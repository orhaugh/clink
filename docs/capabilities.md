...
| Rescale | Change one operator's parallelism on a running job (`clink rescale-op`, HTTP, or the autoscaler): drain, replan from the job graph, redeploy from the last completed checkpoint with key-group state repartitioned. Integer factors, declared bounds required, and a stop rather than a seamless cutover. Whole-job rescale by role is refused for multi-operator jobs | [Fault tolerance and rescale](internals/fault-tolerance-and-rescale.md) |
