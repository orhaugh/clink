...
| Rescale | Key-group repartitioning on deploy, scale up and down at a new parallelism. Live rescale of a running job is limited: whole-job rescale is refused for multi-operator jobs, and per-operator rescale (and so the autoscaler that drives it) is not executable on a planned job - both refuse with a reason. Change a running job's parallelism by savepoint and resubmit | [Fault tolerance and rescale](internals/fault-tolerance-and-rescale.md) |
