...
| Memory budgets | Opt-in shared accounting for local edge queues, SQL GROUP BY and tumble/hop/cumulate working state, memory/file backends and checkpoint buffers; blocking exchanges and synchronous SQL GROUP BY spill on pressure when configured, with budgeted exchange and SQL TTL metadata. Other covered owners refuse on exhaustion. Partial allocation coverage, not an RSS cap; cluster limits are per local execution | [Memory budgets](internals/memory-management.md) |
