...
| Memory budgets | Opt-in shared accounting for local queues, covered SQL aggregate/window/join/OVER/ranking state, scalar-subquery and model-inference input buffers, memory/file backends and checkpoint buffers. Blocking exchanges and covered synchronous SQL partitions spill on pressure when configured. Global top-N and null wildcard entries can also spill; TTL indexes stay in RAM. Entry-level GROUP BY, window/session, OVER, ranking, join and scalar-buffer storage supports oversized active keys or inputs; built-in GROUP BY, fixed-window and session value collections split into individual cells. Batched model inference flushes early under pressure. Individual rows, value cells, materialised results and opaque aggregate accumulators must still fit. Partial allocation coverage, not an RSS cap; cluster limits are per local execution | [Memory budgets](internals/memory-management.md) |
