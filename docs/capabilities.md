...
| High availability | Multi-coordinator leader election via etcd (opt-in); filesystem-backed job persistence | [Distributed runtime](internals/distributed-runtime.md) |
...
| Efficiency | Measured 3.04x less CPU per event and 16.4x less memory than a JVM stream processor on identical hardware with byte-identical output; raw per-run data published | [Efficiency and environmental impact](efficiency.md) |
