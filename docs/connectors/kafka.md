...
The `KafkaSource::Options` struct also carries `poll_timeout` (100 ms), `max_batch_size` (2048), `commit_mode` (`Auto`), `enable_debug` (false), `metric_prefix` (`default`), `partition_discovery_interval` (30 s; 0 disables) and the deterministic-ownership pair `subtask_index`/`source_parallelism` (0/1, meaning a standalone source owns every partition). The ownership pair is set by the registered factories from the build context; the others are not parsed from `BuildContext` parameters, so they take their struct defaults unless the typed class is constructed directly.
