...
- Source replay (exactly-once on the read side): the source records a per-partition next-offset map into the clink checkpoint via `snapshot_offset`, and on `restore_offset` it seeks each partition to the restored offset on assignment through a rebalance callback. This makes the clink checkpoint the source of truth for the consumer position on recovery rather than Kafka's own committed offset. At parallelism greater than one, the registered factories also give every source subtask a deterministic Kafka `group.instance.id`, scoped by topic and subtask index. Kafka therefore maps a restored subtask back to its former static group member and preserves its partition ownership; without that identity, a full coordinator recovery can exchange partitions between subtasks and pair one subtask's checkpointed window state with another subtask's source offset. An explicit `kafka.group.instance.id` passthrough still wins, and must be unique within the consumer group. The `StringKafkaSource` adapter delegates the checkpoint hooks to the inner `KafkaSource` so the string and SQL paths retain replay.
