#!/usr/bin/env bash
# QUAL-09: the infrastructure fault matrix.
#
# THE CLAIM. Exactly-once output and bounded, diagnosable recovery hold
# under INFRASTRUCTURE faults - a state volume at capacity, network
# partitions sustained past every recovery deadline, and stepped host
# clocks - with each fault proven to have ENGAGED (not merely fired) and
# the standard every-key oracle exact at the end.
#
# WHY ENGAGEMENT GATES. An infra fault that did not bite proves nothing:
# a disk-pressure window in which no checkpoint even tried to fail, a
# "sustained" partition the watchdog never noticed, a clock step nobody
# measured - each would be recorded as survived-a-fault while testing
# air. Every infra fault therefore records its own evidence (failed-
# checkpoint counter deltas, worker-loss counter deltas plus the rejoin,
# measured clock offsets) and the summariser refuses coverage credit
# without it. The retention-engaged discipline from QUAL-05, generalised.
#
# ENVIRONMENT SPLIT, stated rather than smuggled. clock_step needs a
# host whose kernel is its own (the local rig's docker-in-docker hosts
# share the laptop VM's), and disk_pressure needs a state filesystem
# that is a bounded sandbox (the cloud rig mounts /qual/state from a
# loopback image; the local rig's shared volume IS the laptop's disk,
# and this morning proved what filling that does). The local campaign
# passes both on --skip-faults - recorded by the controller, tolerated
# by the summariser only under --local, and pinned by controller unit
# tests either way. A LOCAL PASS is the harness gate; the CLAIM is only
# ever made from a cloud run with full coverage.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$HERE/../.." && pwd)"

RUN_ID="${RUN_ID:?set RUN_ID, e.g. qual09-20260825a}"
...
# --- the battery ---------------------------------------------------------------
echo "campaign: === infrastructure battery (${DURATION_S}s) ==="
Q9_POINTS=$(python3 -c "
...
python3 "$HERE/summarise.py" --out-dir "$OUT_DIR" --run-id "$RUN_ID" \
    --profile "$PROFILE" $LOCAL_ARG > "$OUT_DIR/QUAL-09-summary.md"
cat "$OUT_DIR/QUAL-09-summary.md"
