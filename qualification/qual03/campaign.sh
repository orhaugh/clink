#!/usr/bin/env bash
# QUAL-03: S3 staged-commit sink under worker and store faults.
#
# QUAL-01 qualifies exactly-once into Kafka, where the transaction is the
# broker's; QUAL-02 the external transaction manager, where Postgres holds
# a prepared transaction. This campaign qualifies the third delivery
# shape: the STAGED ARTIFACT, where clink buffers a checkpoint interval,
# uploads it as multipart parts at the barrier, and makes the object
# visible atomically at CompleteMultipartUpload only once the checkpoint
# is durable. The interesting window is between barrier and commit - a
# worker killed there leaves a staged upload that outlives its uploader,
# and recovery must complete it in exactly one direction, including
# against a store that is DOWN when the recovery needs it.
#
# The oracle reads the bucket directly and never asks clink anything. See
# verifier.py for the five countable failure modes.
#
#   RUN_ID=qual03-YYYYMMDD [INVENTORY=<path>] ./campaign.sh
#
...
#
# This driver carries every harness lesson QUAL-01 and QUAL-02 paid for:
# docker and broker readiness gates, the NFS shared-state probe, a per-run
# checkpoint directory under the shared mount, the fault-surface gate, the
...
# judgement, an end-state completeness capture by a FRESH full re-read
# (the incremental oracle's own ledger must not certify itself), and a
# summary that refuses PASS without coverage and stability evidence.
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$HERE/../.." && pwd)"

RUN_ID="${RUN_ID:?set RUN_ID (e.g. qual03-20260822)}"
...
S3_ACCESS_KEY="qual03"
S3_SECRET_KEY="${S3_SECRET_KEY:-qual03-$(echo "$RUN_ID" | tr -dc 'a-z0-9')}"
S3_BUCKET="qual03"
S3_PREFIX="q3"
...
rm -f "$OUT_DIR/q3-verdict.json" "$OUT_DIR/q3-chaos.jsonl" "$OUT_DIR/q3-progress.json" \
...
echo "campaign: QUAL-03 run $RUN_ID, ${DURATION_H}h, profile=$PROFILE"
...
# --- sink object store ----------------------------------------------------
echo "campaign: starting the sink object store"
...
# down -v FIRST: MinIO's root credentials only take effect when the data
# directory is initialised, and this campaign derives its secret from
# RUN_ID - so a second campaign on a REUSED rig inherits a volume keyed
# to the previous run's secret and every store call fails
# InvalidAccessKeyId. A fresh volume is also the right default on its own
# terms: the bucket's contents are this campaign's evidence, and
# inheriting another run's objects corrupts them (the same reasoning that
# wipes the HA job store between runs).
...
on_host "$OPS_PUB" "curl -fsS $S3_ENDPOINT_OPS/minio/health/ready >/dev/null 2>&1" \
    || { echo "campaign: sink object store never became ready" >&2; exit 2; }

# Bucket creation over the S3 API itself (curl's sigv4), so the only
# credential path exercised is the one the sink and the oracle use.
on_host "$OPS_PUB" "curl -fsS -X PUT --aws-sigv4 'aws:amz:us-east-1:s3' \
    ...
    || { echo "campaign: could not create the sink bucket" >&2; exit 2; }
echo "campaign: sink bucket created (fresh volume - no unique constraint exists in an object store; duplicates land as repeated lines and the oracle must see them)"
...
rpk_ops "topic delete qual03-in" >/dev/null 2>&1 || true
...
for _ in $(seq 1 30); do
    if ! rpk_ops "topic list" 2>/dev/null | awk '{print $1}' | grep -qx qual03-in; then
        gone=1; break
    fi
    rpk_ops "topic delete qual03-in" >/dev/null 2>&1 || true
    sleep 2
done
[ "$gone" = "1" ] || { echo "campaign: topic qual03-in still exists after 30 delete attempts" >&2; exit 2; }
rpk_ops "topic create qual03-in -p $PARTITIONS -r $REPLICATION"
...
on_host "$OPS_PUB" "rm -f /qual/q3-progress.json.stop /qual/q3-chaos.jsonl.stop"
for f in ../qual01/detspec.py ../qual01/generator.py verifier.py s3probe.py endstate.py; do
...
on_host "$OPS_PUB" "chmod 600 /root/.ssh/id_ed25519 && \
    (pip3 install --break-system-packages -q confluent-kafka boto3 \
     || pip3 install -q confluent-kafka boto3)"

on_host "$OPS_PUB" "pkill -f '[g]enerator.py'; pkill -f '[v]erifier.py'; pkill -f '[c]haos.py'; \
    rm -f /qual/q3-progress.json* /qual/q3-verdict.json /qual/q3-chaos.jsonl; true"

# The store's own probes, as the driver's psql_q equivalent.
s3_objects() { on_host "$OPS_PUB" "python3 /qual/s3probe.py objects --endpoint $S3_ENDPOINT_OPS \
    --bucket $S3_BUCKET --prefix $S3_PREFIX --access-key '$S3_ACCESS_KEY' --secret-key '$S3_SECRET_KEY'"; }
s3_uploads() { on_host "$OPS_PUB" "python3 /qual/s3probe.py uploads --endpoint $S3_ENDPOINT_OPS \
    --bucket $S3_BUCKET --prefix $S3_PREFIX --access-key '$S3_ACCESS_KEY' --secret-key '$S3_SECRET_KEY'"; }

# The bucket must start EMPTY - its contents are this run's evidence.
PRE=$(s3_objects | tr -d '\r')
[ "${PRE:-1}" = "0" ] || { echo "campaign: the sink bucket already holds $PRE object(s)" >&2; exit 2; }

BASE_MS=$(( $(date +%s) * 1000 ))
echo "$BASE_MS" > "$OUT_DIR/base_ms"

start_on_host "$OPS_PUB" q3-generator.log "python3 generator.py --brokers '$BROKER_LIST' \
    --topic qual03-in --rate $RATE --partitions $PARTITIONS --keys $KEYS \
    --seed $SEED --base-ms $BASE_MS --max-jitter-ms 0 \
    --window-ms 10000 --progress /qual/q3-progress.json"
echo "campaign: generator started"

# --- pipeline -----------------------------------------------------------
sed -e "s|__BROKERS__|$BROKER_LIST|g" -e "s|__BUCKET__|$S3_BUCKET|g" \
    -e "s|__S3_ENDPOINT__|$S3_ENDPOINT_WORKERS|g" \
...
start_on_host "$OPS_PUB" q3-verifier.log "python3 verifier.py --endpoint $S3_ENDPOINT_OPS \
    --bucket $S3_BUCKET --prefix $S3_PREFIX \
    --access-key '$S3_ACCESS_KEY' --secret-key '$S3_SECRET_KEY' \
    --progress /qual/q3-progress.json --out /qual/q3-verdict.json --interval-s 20"
...
import json;d=json.load(open('/qual/q3-progress.json'));print(sum(d['produced_high'].values()))\"" 2>/dev/null || echo 0)
sleep 30
...
# 3. Objects are being COMMITTED to the store. A staged-commit sink that
#    never completes an upload looks identical to a healthy one from
#    clink's side, so this is read from the bucket.
COMMITTED=0
for _ in $(seq 1 20); do
    COMMITTED=$(s3_objects | tr -d '\r')
    [ "${COMMITTED:-0}" -gt 0 ] && break
    sleep 15
done
[ "${COMMITTED:-0}" -gt 0 ] || verify_fail "no objects have been committed to the store"
echo "campaign: objects committed to the sink bucket ($COMMITTED)"

# 4. The mechanism UNDER TEST is actually engaged.
#
#    Objects landing proves the sink works; it does NOT prove they arrive
#    through staged multipart commits - an at-least-once PutObject sink
...
[ "${UPLOADS_SAMPLED:-0}" -gt 0 ] \
    && echo "campaign: staged upload observed in flight (${UPLOADS_SAMPLED} pending)"
...
Q3_POINTS=$(cd "$(dirname "$0")" && python3 -c "import summarise; print(','.join(summarise.TWOPC_POINTS))")
[ -n "$Q3_POINTS" ] || { echo "campaign: could not derive the 2PC point set from summarise.py" >&2; exit 78; }
start_on_host "$OPS_PUB" q3-chaos.log "python3 chaos.py --inventory /qual/inventory.json \
    --log /qual/q3-chaos.jsonl --coordinator-url http://${COORD_PRIV}:8095 \
    --job-id $JOB_ID --run-id $RUN_ID --profile $PROFILE --seed $SEED \
    --min-gap-s ${MIN_GAP_S:-120} --twopc-points $Q3_POINTS \
    --extra-faults s3_unavailable \
    --duration-s $(( DURATION_S + 1800 )) --ensure-coverage"
echo "campaign: chaos started (${DURATION_H}h, profile=$PROFILE, coverage-first)"

FAULTS=0
for _ in $(seq 1 60); do
    FAULTS=$(on_host "$OPS_PUB" "wc -l < /qual/q3-chaos.jsonl 2>/dev/null || echo 0" | tr -d '\r')
    [ "${FAULTS:-0}" -gt 0 ] && break
    sleep 20
done
...
AFTER=$(s3_objects | tr -d '\r')
[ "${AFTER:-0}" -gt "${BEFORE:-0}" ] \
    || verify_fail "the job reports RUNNING after the fault but has committed no further objects
  ($BEFORE -> $AFTER). A recovered job that cannot commit is not recovered."
echo "campaign: recovered and still committing ($BEFORE -> $AFTER objects) - VERIFICATION PASSED, entering soak"
...
while [ "$(date +%s)" -lt "$END" ]; do
    ...
    sleep 120
    for f in q3-verdict.json q3-chaos.jsonl q3-progress.json q3-generator.log q3-verifier.log q3-chaos.log; do
        scp "${SSH_OPTS[@]}" -q "root@${OPS_PUB}:/qual/$f" "$OUT_DIR/" 2>/dev/null || true
    done

    DIRTY=$(python3 - "$OUT_DIR/q3-verdict.json" <<'PY' || echo ""
import json, sys
try:
...
PY
    ...
    if [ "$DIRTY" = "DIRTY" ]; then
        ...
        on_host "$OPS_PUB" "touch /qual/q3-chaos.jsonl.stop"
    ...
    if [ -z "$CHAOS_DIED_AT" ] \
       && ! on_host "$OPS_PUB" "pgrep -f '[c]haos.py' >/dev/null"; then
        ...
        NFAULTS=$(on_host "$OPS_PUB" "wc -l < /qual/q3-chaos.jsonl 2>/dev/null || echo 0" | tr -d '\r')
        ...
        { echo "chaos_controller_died=yes"; echo "noticed_at_utc=$CHAOS_DIED_AT";
          ...
          echo "tail:"; on_host "$OPS_PUB" "tail -20 /qual/q3-chaos.log" 2>/dev/null || true;
...
try:
    ...
    OBJS=$(s3_objects | tr -d '\r' 2>/dev/null || echo '?')
    echo "campaign: $(date -u +%H:%M) soak, $OBJS objects committed"
done
...
on_host "$OPS_PUB" "docker unpause qual03-minio >/dev/null 2>&1; true"
on_host "$OPS_PUB" "touch /qual/q3-progress.json.stop"
...
while [ "$waited" -lt "$FINAL_WAIT_S" ]; do
    scp "${SSH_OPTS[@]}" -q "root@${OPS_PUB}:/qual/q3-verdict.json" "$OUT_DIR/" 2>/dev/null || true
    lines=$(python3 - "$OUT_DIR/q3-verdict.json" <<'PY' || echo -1
import json, sys
try:
    v = json.load(open(sys.argv[1]))
    print((v.get("last_stats") or {}).get("lines_total", -1))
except Exception:
    print(-1)
PY
)
...
# End-state completeness AND the authoritative foreign check, by a FRESH
# full re-read in a new process (endstate.py). The incremental oracle's
# totals ride its own read ledger; a campaign must not let the oracle
# certify itself. The generator has stopped, so its final progress file
# is now an upper bound and "committed past it" is judgeable - mid-flight
# the verifier deliberately cannot make that call (its snapshot is only a
# lower bound, and treating it as an upper one made QUAL-02's oracle
# invent 32 findings on a provably complete run).
on_host "$OPS_PUB" "python3 /qual/endstate.py --endpoint $S3_ENDPOINT_OPS \
    --bucket $S3_BUCKET --prefix $S3_PREFIX \
    --access-key '$S3_ACCESS_KEY' --secret-key '$S3_SECRET_KEY' \
    --progress /qual/q3-progress.json" > "$OUT_DIR/completeness.txt" \
    || echo "campaign: WARNING - the end-state pass failed; completeness has no evidence" >&2
cat "$OUT_DIR/completeness.txt" 2>/dev/null || true

# Pending uploads sampled BEFORE the job is cancelled, and again after.
# Unlike prepared transactions these are recorded, not gated: an upload
# orphaned by a kill before its checkpoint became durable stays pending
# BY DESIGN and production expires it with a lifecycle rule.
echo "pending_uploads=$(s3_uploads | tr -d '\r')" > "$OUT_DIR/pending-before-cancel.txt" || true
curl -fsS -X POST "http://${COORD_PUB}:8095/api/v1/jobs/${JOB_ID}/cancel" >/dev/null 2>&1 || true
sleep 30
echo "pending_uploads=$(s3_uploads | tr -d '\r')" > "$OUT_DIR/pending-after-cancel.txt" || true
...
collect_container_logs
s3_objects > "$OUT_DIR/objects-final.txt" 2>/dev/null || true
...
python3 "$HERE/summarise.py" --out-dir "$OUT_DIR" --run-id "$RUN_ID" \
    --duration-h "$DURATION_H" --profile "$PROFILE" > "$OUT_DIR/QUAL-03-summary.md"
cat "$OUT_DIR/QUAL-03-summary.md"
